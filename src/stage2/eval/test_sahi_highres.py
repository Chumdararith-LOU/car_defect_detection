...
def run_direct_1024(model, img_path, device, min_conf):
...
def run_sahi_sliced(detection_model, img_path, slice_size, overlap_ratio, min_conf):
    start_time = time.time()
...
def main():
    parser = argparse.ArgumentParser(
        description="High-Res SAHI 1024 vs 640 Comparison Engine"
    )
    parser.add_argument(
    ...
    # Load YOLO for direct downscale
    ...
    print("\n" + "=" * 105)
    print(" 🚀 HIGH-RESOLUTION SAHI BENCHMARK REPORT")
    print("=" * 105)
    ...
    for img_path in sorted(image_files):
        ...
        print(
            f"   ├─ Direct 1024 : {sum(d1024_counts.values()):>2} detections | {d1024_lat:>6.1f} ms"
        )
        print(
            f"   ├─ SAHI 640    : {sum(s640_counts.values()):>2} detections | {s640_lat:>6.1f} ms"
        )
        print(
            f"   └─ SAHI 1024   : {sum(s1024_counts.values()):>2} detections | {s1024_lat:>6.1f} ms"
        )

    print("\n" + "=" * 105)
    print(" 📊 AGGREGATE DETECTION COUNTS BY CLASS")
    print("=" * 105)
    header = (
        f"{'Mode':<15} | "
