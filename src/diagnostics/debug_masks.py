...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Masks Directory Structural Health Check"
    )
    parser.add_argument(
        "--masks_dir",
        type=str,
        default="data/processed/sod/train/masks",
        help="Path to the directory containing ground truth masks to check",
    )
    parser.add_argument(
        "--sample_limit",
        type=int,
        default=100,
        help="Max number of files to sample check",
    )
    ...
    for f in files[: args.sample_limit]:
    ...
    print(
        f"Successfully read by OpenCV:  {successfully_loaded} / {min(args.sample_limit, total_files)} (sampled)"
    )
