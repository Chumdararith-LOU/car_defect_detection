...
def apply_surgical_mode(model, mode):
    ...
    if len(layers) != NUM_LAYERS:
        raise ValueError(
            f"mode '{mode}' expects {NUM_LAYERS} layers, model has {len(layers)}"
        )
...
def self_check():
    ...
    assert UNFREEZE_MODES["early_texture"] == {
        0,
        1,
        2,
        3,
        4,
        23,
    }, "early_texture drifted from seesaw control set"
    keys = list(UNFREEZE_MODES)
    for a, b in zip(keys, keys[1:]):
        assert (
            UNFREEZE_MODES[a] < UNFREEZE_MODES[b]
        ), f"{a} must be a strict subset of {b}"
    assert all(
        HEAD_LAYER in s for s in UNFREEZE_MODES.values()
    ), "head must be unfrozen in every mode"
