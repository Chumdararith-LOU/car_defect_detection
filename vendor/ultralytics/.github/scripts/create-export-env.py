...
def isolated_env_ids():
    ...
    return sorted(
        (env for env, recipe in EXPORT_ENVS.items() if recipe["python"]),
        key=lambda env: env != "isolated-deepx",
    )
...
def build_env(env_id, root):
    ...
    subprocess.run(
        ["uv", "venv", str(venv), "--python", recipe["python"], "--seed"], check=True
    )
    ...
    if recipe["env"]:
        ...
        site_packages.joinpath("sitecustomize.py").write_text(
            "\n".join(
                f'import os; os.environ.setdefault("{k}", "{v}")'
...
def main():
    ...
    parser.add_argument(
        "--list",
        action="store_true",
        help="Print isolated export environment ids and exit.",
    )
