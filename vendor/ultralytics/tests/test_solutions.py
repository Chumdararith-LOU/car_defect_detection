...
REGION = [
    ...
    (540, 180),
    (10, 180),
]  # for object counting, speed estimation and queue management
...
def test_left_click_selection():
    ...
    assert (
        1 in dc.selected_boxes
    ), f"Expected track_id 1 in selected_boxes, got {dc.selected_boxes}"
...
def test_right_click_reset():
    ...
    assert (
        not dc.selected_boxes
    ), f"Expected empty selected_boxes after reset, got {dc.selected_boxes}"
    assert (
        dc.left_mouse_count == 0
    ), f"Expected left_mouse_count=0 after reset, got {dc.left_mouse_count}"
...
def test_analytics_graph_not_supported():
    ...
    try:
        ...
        assert "Unsupported analytics_type" in str(
            e
        ), f"Expected 'Unsupported analytics_type' in error, got: {e}"
...
def test_area_chart_padding():
    ...
    plot_im = analytics.update_graph(
        frame_number=2, count_dict={"car": 3, "person": 1}, plot="area"
    )
...
def test_config_update_method_with_invalid_argument():
    ...
    try:
        ...
        assert "is not a valid solution argument" in str(
            e
...
@pytest.mark.skipif(
    not TORCH_2_4,
    reason=f"VisualAISearch requires torch>=2.4 (found torch=={TORCH_VERSION})",
)
@pytest.mark.skipif(
    IS_RASPBERRYPI, reason="Disabled due to slow performance on Raspberry Pi."
)
def test_similarity_search(tmp_path):
    """Test similarity search solution with sample images and text query."""
    safe_download(
        f"{ASSETS_URL}/4-imgs-similaritysearch.zip", dir=tmp_path
    )  # 4 dog images for testing in a zip file
    searcher = solutions.VisualAISearch(data=str(tmp_path / "4-imgs-similaritysearch"))
    _ = searcher(
        "a dog sitting on a bench"
    )  # Returns the results in format "- img name | similarity score"
...
def test_distance_calculation_process_method():
    ...
    with patch.object(dc, "extract_tracks"), patch.object(dc, "display_output"), patch(
        "cv2.setMouseCallback"
    ):
        result = dc.process(frame)
    assert isinstance(
        result, SolutionResults
    ), f"Expected SolutionResults, got {type(result)}"
    assert result.total_tracks == 2, f"Expected 2 tracks, got {result.total_tracks}"
    assert (
        result.pixels_distance > 0
    ), f"Expected positive distance, got {result.pixels_distance}"
...
def test_display_output_method():
    ...
    ) as mock_wait, patch("cv2.destroyAllWindows") as mock_destroy:
