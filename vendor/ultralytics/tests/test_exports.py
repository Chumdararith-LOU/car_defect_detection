...
def skip_rpi_semantic(task):
    ...
    if IS_RASPBERRYPI and task == "semantic":
        pytest.skip(
            "Semantic segmentation export tests are skipped on Raspberry Pi due to memory constraints."
        )
...
def test_export_torchscript(end2end, isolated_model):
    ...
    file = YOLO(isolated_model).export(
        format="torchscript", optimize=False, imgsz=32, end2end=end2end
    )
...
def test_export_onnx(end2end, isolated_model):
    ...
    file = YOLO(isolated_model).export(
        format="onnx", dynamic=True, imgsz=32, end2end=end2end
    )
    YOLO(file)(SOURCE, imgsz=32)  # exported model inference
...
def test_export_onnx_int8(isolated_model, precision):
    ...
    file = YOLO(isolated_model).export(
        format="onnx", data="coco8.yaml", fraction=0.25, imgsz=32, **precision
    )
...
def test_quantize_deprecation():
    ...
    assert (
        _handle_deprecation({"half": True, "int8": True})["quantize"] == 8
    )  # int8 wins
    assert "half" not in _handle_deprecation(
        {"half": True}
    )  # legacy flag is removed after forwarding
    ...
    def export_model(index: int):
        try:
            torch2onnx(
                torch.nn.Identity(),
                torch.zeros(1, 3, 8, 8),
                str(tmp_path / f"export-{index}.onnx"),
            )
...
def test_export_openvino_matrix(task, dynamic, quantize, batch, nms, end2end):
    ...
    file = YOLO(TASK2MODEL[task]).export(
        ...
        data=TASK2DATA[
            task
        ],  # use the smallest task datasets for fast INT8 calibration
        nms=nms,
        end2end=end2end,
    )
    if WINDOWS:
        ...
        file = file.rename(file.with_stem(f"{file.stem}-{uuid.uuid4()}"))
    YOLO(file)(
        [SOURCE] * batch, imgsz=64 if dynamic else 32, batch=batch
...
@pytest.mark.skipif(
    MACOS and MACOS_VERSION and MACOS_VERSION >= "15",
    reason="CoreML YOLO26 matrix test crashes on macOS 15+",
)
@pytest.mark.parametrize(
...
def test_export_coreml(isolated_model):
    ...
    assert (
        "You will not be able to run predict()" not in output
    ), "CoreML export has predict() error"
...
def test_export_coreml_rtdetr():
    ...
    assert (
        "You will not be able to run predict()" not in output
    ), "RTDETR CoreML export has predict() error"
...
@pytest.mark.parametrize(
    ...
    [  # generate all combinations except for exclusion cases
        (task, quantize, batch, end2end)
        for task, quantize, batch, end2end in product(
            sorted(TASKS), [8, 16], [1, 2], [True, False]
        )
    ],
)
...
@pytest.mark.skipif(
    not TORCH_2_0, reason="NCNN inference causes segfault on PyTorch<2.0"
)
...
@pytest.mark.skipif(
    not checks.IS_PYTHON_MINIMUM_3_9, reason="IMX export requires Python>=3.9"
)
@pytest.mark.skipif(not LINUX, reason="IMX export only supported on Linux")
@pytest.mark.skipif(
    IS_RASPBERRYPI,
    reason="Test disabled as IMX export suffers from OOM (Out of Memory) on Raspberry Pi 5 16GB",
)
def test_export_imx():
...
@pytest.mark.slow
@pytest.mark.skipif(
    not LINUX or ARM64, reason="RKNN export only supported on non-aarch64 Linux"
)
@pytest.mark.parametrize("quantize", [8, 16])
def test_export_rknn(isolated_model, quantize):
    """Test YOLO export to RKNN format."""
    file = YOLO(isolated_model).export(
        format="rknn", imgsz=32, quantize=quantize, data="coco8.yaml"
    )
    assert next(
        Path(file).rglob("*.rknn"), None
    ), f"RKNN export failed, no RKNN model found in: {file}"
    shutil.rmtree(file, ignore_errors=True)


# @pytest.mark.skipif(True, reason="Disabled for debugging ruamel.yaml installation required by executorch")
@pytest.mark.skipif(
    not checks.IS_PYTHON_MINIMUM_3_10 or not TORCH_2_9,
    reason="Requires Python>=3.10 and Torch>=2.9.0",
)
...
def test_export_executorch(isolated_model):
    ...
    ), f"ExecuTorch metadata.yaml not found: {metadata_file}"
...
@pytest.mark.slow
@pytest.mark.skipif(
    not (MACOS or (LINUX and not ARM64)),
    reason="LiteRT export only supported on Linux x86 and macOS",
)
@pytest.mark.skipif(
    not checks.IS_PYTHON_MINIMUM_3_10, reason="litert-torch requires Python>=3.10"
)
@pytest.mark.parametrize(
    "task, quantize",
    [
...
def test_export_litert_matrix(task, quantize):
    ...
    file = Path(
        YOLO(TASK2MODEL[task]).export(format="litert", imgsz=32, quantize=quantize)
    )
    assert (
        file.is_file() and file.suffix == ".tflite"
    ), f"LiteRT export is not a single .tflite for '{task}': {file}"
    ...
    assert all(
        d["dtype"] == np.float32 for d in io_details
    ), f"LiteRT '{task}' quantize={quantize} must keep float32 I/O, got {[d['dtype'] for d in io_details]}"
    YOLO(file)(
        SOURCE, imgsz=32
    )  # exported model inference (also exercises the embedded metadata)
    file.unlink()  # cleanup
...
def test_export_executorch_matrix(task):
    ...
    assert Path(
        ...
        f"ExecuTorch export failed for task '{task}', directory not found: {file}"
    )
    # Check that .pte file exists in the exported directory
    pte_file = Path(file) / "model.pte"
    assert (
        pte_file.exists()
    ), f"ExecuTorch .pte file not found for task '{task}': {pte_file}"
    # Check that metadata.yaml exists
    metadata_file = Path(file) / "metadata.yaml"
    assert (
        metadata_file.exists()
    ), f"ExecuTorch metadata.yaml not found for task '{task}': {metadata_file}"
    # Note: Inference testing skipped as ExecuTorch requires special runtime setup
    shutil.rmtree(file, ignore_errors=True)  # cleanup
...
def test_export_qnn(isolated_model):
    ...
    assert Path(file).is_file() and file.endswith(
        "_qnn.onnx"
    ), f"QNN export failed, no context binary found: {file}"
...
@pytest.mark.parametrize(
    "env", [k for k, v in EXPORT_ENVS.items() if k != "base" or v["smoke"]]
)
