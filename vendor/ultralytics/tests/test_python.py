...
def test_dataloader_caps_workers_to_batches():
    ...
    drop_last_single_batch = build_dataloader(
        range(5), batch=4, workers=8, drop_last=True
    )
...
def test_dataloader_cap_preserves_distributed_drop_last(monkeypatch):
    ...
    monkeypatch.setattr(
        data_build.distributed, "DistributedSampler", distributed_sampler
    )
...
def skip_rpi_semantic():
    ...
    if IS_RASPBERRYPI:
        pytest.skip(
            "Semantic segmentation tests are skipped on Raspberry Pi due to memory constraints."
        )
...
def test_select_device(monkeypatch):
    ...
    assert (
        not set_calls
    )  # default '' request must never move the current device, e.g. diagnostics like check_yolo()
    for _ in range(
        2
    ):  # repeated calls are idempotent, e.g. Trainer.__init__ then final_eval, or predict() twice
        ...
        )  # CUDA_VISIBLE_DEVICES never written
    assert set_calls == [
        1,
        1,
    ]  # explicit single-GPU requests set the default device for indexless 'cuda' operations
    assert str(torch_utils.select_device("0,1", verbose=False)) == "cuda:0"
    assert set_calls == [
        1,
        1,
    ]  # multi-GPU requests never move the current device; DDP ranks pin theirs in _setup_ddp
    monkeypatch.setattr(torch_utils.torch.cuda, "current_device", lambda: 1)
    assert (
        str(torch_utils.select_device("", verbose=False)) == "cuda:1"
    )  # default '' resolves to the current device
    assert (
        str(torch_utils.select_device(torch.device("cuda", 1), verbose=False))
        == "cuda:1"
    )
    with pytest.raises(
        ValueError
    ):  # torch.device inputs are validated like strings, no raw CUDA errors
        torch_utils.select_device(torch.device("cuda", 3), verbose=False)
    set_calls.clear()
    assert (
        str(torch_utils.select_device(torch.device("cuda"), verbose=False)) == "cuda:1"
    )
    assert (
        not set_calls
    )  # indexless torch.device('cuda') means the current device and never moves it
    assert torch_utils.parse_device([0, 1]) == "0,1"
    assert (
        torch_utils.parse_device("00,01") == "0,1"
    )  # leading zeros stripped for valid torch device strings
    assert (
        torch_utils.parse_device(torch.device("cuda")) == ""
    )  # indexless 'cuda' stays the '' default request
    ...
    assert (
        str(torch_utils.select_device("3", verbose=False)) == "cuda:0"
    )  # e.g. pods launched with CVD preset
    assert (
        torch_utils.parse_device(torch_utils.parse_device("3")) == "0"
    )  # idempotent: trainer + select_device parse
    ...
    monkeypatch.setattr(
        autodevice.GPUInfo,
        "__init__",
        lambda self: self.__dict__.update(nvml_available=False),
    )
    monkeypatch.setattr(
        autodevice.GPUInfo,
        "select_idle_gpu",
        lambda self, count=1, indices=None, **kw: [
            i for i in (0, 1, 3) if i in indices
        ][:count],
    )
    monkeypatch.setattr(torch_utils.torch.cuda, "device_count", lambda: 2)
    monkeypatch.setenv("CUDA_VISIBLE_DEVICES", "1,3")
    assert (
        torch_utils.parse_device("-1") == "0"
    )  # idle physical GPU 1 is torch index 0 under CVD='1,3'; 0 is hidden
    assert (
        torch_utils.parse_device("1") == "1"
    )  # in-range ids are torch indices, so repeated parses are stable
    assert (
        torch_utils.parse_device("-1,3") == "0,1"
    )  # mixed: idle physical GPU 1 + physical GPU 3 as torch indices
    assert (
        torch_utils.parse_device("0,1") == "0,1"
    )  # already-translated outputs re-parse unchanged (idempotent)
    monkeypatch.setenv(
        "CUDA_VISIBLE_DEVICES", "3,2,9,1"
    )  # malformed: CUDA stops at the invalid id 9, 2 usable GPUs
    assert (
        torch_utils.parse_device("9") == "9"
    )  # unusable id is not translated, so select_device rejects it
    assert (
        torch_utils.parse_device("2") == "1"
    )  # physical GPU 2 is torch index 1 of the usable prefix
    monkeypatch.setenv(
        "CUDA_VISIBLE_DEVICES", "01,03"
    )  # leading zeros are valid for CUDA's atoi-style parsing
    assert (
        torch_utils.parse_device("3") == "1"
    )  # visible ids normalize like requested ids
    assert (
        torch_utils.parse_device("-1") == "0"
    )  # idle physical GPU 1 found via normalized visible ids
...
def test_predict_csv_multi_row(tmp_path):
    ...
    assert (
        len(results) == 7
    ), f"Expected 7 results from multi-row CSV, got {len(results)}"
...
def test_predict_csv_single_row(tmp_path):
    ...
    assert (
        len(results) == 7
    ), f"Expected 7 results from single-row CSV, got {len(results)}"
...
def test_predict_img(model_name):
    ...
    batch = [
        ...
        Path(SOURCE),  # Path
        (
            "https://cdn.jsdelivr.net/gh/ultralytics/assets@main/im/zidane.jpg?token=123"
            if ONLINE
            else SOURCE
        ),  # URI
        im,  # OpenCV
...
def test_predict_gray_and_4ch(tmp_path):
    ...
    for f in source_rgba, source_grayscale, source_non_utf, source_spaces:
        for source in Image.open(f), cv2.imread(str(f)), f:
            ...
            assert (
                len(results) == 1
            ), f"Expected 1 result for {f.name}, got {len(results)}"
        f.unlink()  # cleanup
...
def test_predict_grayscale_ndarray():
    ...
    assert (
        len(model(source=gray, imgsz=32, verbose=False)) == 1
    )  # 2D ndarray auto-expanded to 3 channels
    assert (
        len(model(source=gray.astype("float64"), imgsz=32, verbose=False)) == 1
    )  # non-OpenCV dtype also works
...
def test_predict_all_image_formats():
    ...
    images = [
        im
        for im in (dataset_path / "images" / "train").glob("*.*")
        if im.suffix.lower().lstrip(".") in expected
    ]
    images += [
        im
        for im in (dataset_path / "images" / "val").glob("*.*")
        if im.suffix.lower().lstrip(".") in expected
    ]
...
def test_track_second_association_indices():
    ...
    args = IterableSimpleNamespace(
        **{**YAML.load(ROOT / "cfg/trackers/bytetrack.yaml"), "fuse_score": False}
    )
    ...
    for confs in (
        [0.9, 0.9, 0.9],
        [0.9, 0.9, 0.2],
    ):  # third detection drops to low confidence on frame 2
        data = torch.tensor(
            [[*b, c, 0] for b, c in zip(boxes, confs)], dtype=torch.float32
        )
        tracks = tracker.update(Boxes(data, (640, 640)))
    low = tracks[
        np.isclose(tracks[:, 5], 0.2)
    ]  # columns are [x1, y1, x2, y2, id, score, cls, idx]
    assert (
        len(low) == 1 and int(low[0, -1]) == 2
    ), f"second-association idx not preserved:\n{tracks}"
...
def test_track_second_association_low_conf_keeps_id(tracker_type):
    ...
    args = IterableSimpleNamespace(
        **YAML.load(ROOT / f"cfg/trackers/{tracker_type}.yaml")
    ...
    frame1 = tracker.update(
        Boxes(torch.tensor([[*box, 0.9, 0]], dtype=torch.float32), (640, 640))
    )
    frame2 = tracker.update(
        Boxes(torch.tensor([[*box, 0.15, 0]], dtype=torch.float32), (640, 640))
    )
    ...
    assert (
        len(frame2) == 1
    ), f"low-confidence detection lost by second association:\n{frame2}"
    assert (
        int(frame2[0, 4]) == tid
    ), f"id switched on low-confidence frame: {tid} -> {int(frame2[0, 4])}\n{frame2}"
...
def test_track_stream(model, tmp_path):
    ...
    if model in {
        "yolo26n-cls.pt",
        "yolo26n-sem.pt",
    }:  # classification and semantic segmentation not supported
        return
    from ultralytics.trackers.track import TRACKER_MAP
    ...
    if model == "yolo26n.pt":
        ...
        YAML.save(
            custom_yaml,
            {**default_args, "with_reid": True, "model": "yolo26n-reid.onnx"},
        )
        model.track(video_url, imgsz=160, tracker=custom_yaml)
...
def test_train_multi():
    ...
    assert (
        isinstance(results, dict) and len(results) == 2
    )  # one entry per run (coco8, coco8-2), no duplicate collapse
    assert all(
        m and "fitness" in m for m in results.values()
    )  # checkpoint train metrics per run
    assert len(model.trainer.trainers) == 2  # both list entries fine-tuned in series
    sweep_dir = model.trainer.save_dir
    assert sweep_dir.name.startswith(
        "multitrain"
    )  # all runs grouped under one sweep directory
    assert (
        sweep_dir / "multitrain_results.json"
    ).exists()  # results JSON for post-processing
...
def test_normalize_platform_uri():
    ...
    assert (
        normalize_platform_uri(f"{base}/datasets/coco8")
        == "ul://glenn-jocher/datasets/coco8"
    )
    assert (
        normalize_platform_uri(f"{base}/project/model/")
        == "ul://glenn-jocher/project/model"
    )
    assert (
        normalize_platform_uri("coco8.yaml") == "coco8.yaml"
    )  # non-Platform inputs unchanged
...
def test_results(model: str, tmp_path):
    ...
    if IS_RASPBERRYPI and model == "yolo26n-sem.pt":
        skip_rpi_semantic()
    im = (
        "https://cdn.jsdelivr.net/gh/ultralytics/assets@main/im/boats.jpg"
    ...
    for r in results:
        if is_semantic:
            assert (
                r.semantic_mask is not None and r.semantic_mask.shape == r.orig_shape
            ), f"'{model}' semantic_mask should match the original image shape!"
            assert (
                r.semantic_mask.data.dtype == torch.uint8
            ), f"'{model}' semantic_mask should use compact class IDs!"
        else:
...
def test_labels_and_crops():
    ...
    results = YOLO(WEIGHTS_DIR / "yolo26n.pt")(
    ...
    for r in results:
        ...
        assert (
            len(cls_idxs) >= 2
        ), f"Expected at least 2 detections, got {len(cls_idxs)}"
        # Check label path
        ...
        assert (
            len(r.boxes.data) == label_count
        ), f"Box count {len(r.boxes.data)} != label count {label_count}"
        ...
        assert all(
            r.names.get(c) in crop_dir_names for c in cls_idxs
        ), f"Crop dirs {crop_dir_names} don't match classes {cls_idxs}"
        # Same number of crops as detections
        crop_count = len([f for f in crop_files if im_name in f.name])
        assert crop_count == len(
            r.boxes.data
        ), f"Crop count {crop_count} != detection count {len(r.boxes.data)}"
...
def test_safe_download_unzips_local_path_archive(tmp_path):
    ...
    extracted = safe_download(
        archive, dir=tmp_path / "datasets", unzip=True, progress=False
    )
    expected_path = tmp_path / "datasets" / dataset_dir.name
    assert (
        extracted == expected_path
    ), f"Extracted path {extracted} != expected {expected_path}"
    assert (extracted / "data.yaml").is_file(), f"data.yaml not found in {extracted}"
    assert (
        extracted / "images" / "val"
    ).is_dir(), f"images/val not found in {extracted}"
...
def test_cfg_init():
    ...
    assert (
        smart_value("__import__('os').system('ls')") == "__import__('os').system('ls')"
    )
...
def test_utils_checks():
    ...
    with pytest.raises(ValueError):
        checks.check_imgsz(
            "640x480"
        )  # malformed imgsz string raises a helpful ValueError, not a raw SyntaxError
    ...
    assert checks.check_version(
        "6.0", ">=6.0.0"
    )  # 2-component current must satisfy 3-component requirement
...
def test_utils_benchmarks():
    ...
    ProfileModels(
        ["yolo26n.yaml"], imgsz=32, min_time=1, num_timed_runs=3, num_warmup_runs=1
    ).run()
...
def test_utils_ops():
    ...
    assert segment2box(np.array([[0, 100], [0, 150], [0, 200]]), 640, 640).tolist() == [
        0,
        100,
        0,
        200,
...
def test_process_mask_empty():
    ...
    protos, coeffs, bboxes = (
        torch.rand(32, 160, 160),
        torch.zeros(0, 32),
        torch.zeros(0, 4),
    )
    assert ops.process_mask(
        protos, coeffs, bboxes, (640, 640), upsample=True
    ).shape == (0, 640, 640)
    assert ops.process_mask(protos, coeffs, bboxes, (640, 640)).shape == (
        0,
        160,
        160,
    )  # prototype res when no upsample
    assert ops.process_mask_native(protos, coeffs, bboxes, (640, 640)).shape == (
        0,
        640,
        640,
    )
    assert ops.scale_masks(torch.zeros(1, 0, 160, 160), (640, 640)).shape == (
        1,
        0,
        640,
        640,
...
@pytest.mark.slow
@pytest.mark.skipif(
    IS_RASPBERRYPI or IS_JETSON, reason="Edge devices not intended for tuning"
)
@pytest.mark.skipif(not ONLINE, reason="environment is offline")
def test_model_tune():
...
@pytest.mark.skipif(
    not ONLINE or not checks.IS_PYTHON_MINIMUM_3_10, reason="environment is offline"
)
@pytest.mark.skipif(
    not checks.check_requirements("ray", install=False),
...
def test_model_embeddings():
    ...
    assert model_classify.predict(SOURCE, imgsz=32)[0].probs is not None
    assert isinstance(
        model_classify.predict(SOURCE, imgsz=32, embed=[-2])[0], torch.Tensor
    )
    assert model_classify.predict(SOURCE, imgsz=32)[0].probs is not None
...
def test_process_mask_native_chunked():
    ...
    bboxes = (
        torch.rand(70, 4) * 900 + 5
    )  # fractional boxes exercise the crop edge handling
    bboxes[:, 2:] += bboxes[:, :2]
    out = ops.process_mask_native(
        protos, masks_in, bboxes, (1000, 1000)
    )  # large shape forces multiple chunks
    ref = ops.scale_masks(
        (masks_in @ protos.float().view(32, -1)).view(-1, 160, 160)[None], (1000, 1000)
    )[0]
    ref = (
        ops.crop_mask(ref, bboxes).gt_(0.0).byte()
    )  # single-shot upsample-crop-threshold
    assert torch.equal(out, ref)


@pytest.mark.skipif(
    IS_RASPBERRYPI, reason="Edge devices not intended for CLIP-based models"
)
@pytest.mark.skipif(
...
@pytest.mark.skipif(
    IS_RASPBERRYPI, reason="Edge devices not intended for heavy CLIP-based models"
)
