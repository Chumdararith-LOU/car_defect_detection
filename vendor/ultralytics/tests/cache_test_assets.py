...
def cache_datasets() -> None:
    ...
    safe_download(
        f"{ASSETS_URL}/instances_val2017.json", dir=DATASETS_DIR / "annotations"
    )
...
def cache_clip_model() -> None:
    ...
    if (
        IS_RASPBERRYPI
        or checks.IS_PYTHON_3_12
        or (checks.IS_PYTHON_3_8 and LINUX and ARM64)
    ):
        return
...
def parse_args() -> bool:
    ...
    parser.add_argument(
        "--slow", action="store_true", help="Include assets used only by slow tests."
    )
