...
def test_tensorboard():
    ...
    YOLO("yolo26n-cls.yaml").train(
        data="imagenet10", imgsz=32, epochs=3, plots=False, device="cpu"
    )
    SETTINGS["tensorboard"] = False


@pytest.mark.skipif(
    not check_requirements("ray", install=False), reason="ray[tune] not installed"
)
...
@pytest.mark.skipif(
    not check_requirements("mlflow", install=False), reason="mlflow not installed"
)
...
def test_mlflow_keep_run_active(tmp_path, monkeypatch):
    ...
    try:
        ...
        YOLO("yolo26n-cls.yaml").train(
            data="imagenet10", imgsz=32, epochs=1, plots=False, device="cpu"
        )
        active = mlflow.active_run()
        assert (
            active is not None and active.info.status == "RUNNING"
...
@pytest.mark.skipif(
    not check_requirements("tritonclient", install=False),
    reason="tritonclient[all] not installed",
)
def test_triton(tmp_path, isolated_model):
    ...
    YOLO(f"http://localhost:8000/{model_name}", "detect")(
        SOURCE
...
@pytest.mark.skipif(
    not check_requirements("faster-coco-eval", install=False),
    reason="faster-coco-eval not installed",
)
def test_faster_coco_eval():
    ...
    download(
        f"{ASSETS_URL}/instances_val2017.json", dir=DATASETS_DIR / "coco8/annotations"
    )
    ...
    validator.is_coco = True
    download(
        f"{ASSETS_URL}/instances_val2017.json",
        dir=DATASETS_DIR / "coco8-seg/annotations",
    )
    _ = validator.eval_json(validator.stats)
    ...
    validator.is_coco = True
    download(
        f"{ASSETS_URL}/person_keypoints_val2017.json",
        dir=DATASETS_DIR / "coco8-pose/annotations",
    )
    _ = validator.eval_json(validator.stats)
