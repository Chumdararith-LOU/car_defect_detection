...
def test_export(monkeypatch, tmp_path):
    ...
    assert (
        test_func in exporter.callbacks["on_export_start"]
    ), "on_export_start callback not registered"
...
def test_task(trainer_cls, validator_cls, predictor_cls, data, model, weights):
    ...
    assert (
        test_func in trainer.callbacks["on_train_start"]
    ), "on_train_start callback not registered"
    trainer.train()
    ...
    ), "on_val_start callback not registered"
    ...
    assert (
        test_func in pred.callbacks["on_predict_start"]
    ), "on_predict_start callback not registered"
...
def test_resume_incomplete(task, weight, data, tmp_path):
    ...
    assert (
        resume_model.trainer.start_epoch == resume_model.trainer.epoch == 1
    ), "resume test failed"
...
def test_distill_resume(tmp_path: Path):
    ...
    assert ckpt["epoch"] == 0, "checkpoint should be resumable"
    assert isinstance(
        ckpt["ema"], DistillationModel
    ), "distillation EMA wraps the student model"
    assert (
        ckpt["ema"].teacher_model is None
    ), "teacher should be stripped from the EMA/checkpoint"
    assert (
        ckpt["ema"].projector is not None
    ), "the distillation projector should be persisted in the EMA checkpoint"
    ...
    ), "resume should rebuild the DistillationModel"
    assert (
        model.teacher_model is not None
    ), "resume should rebuild the teacher from the distill_model path"
...
@pytest.mark.parametrize(
    ...
    [
        {
            "model": OrderedDict([("a", torch.zeros(1))])
        },  # state_dict saved under the "model" key
...
def test_checkpoint_fp16_overflow():
    ...
    assert (
        trainer.last.exists()
    ), "checkpoint not saved for a finite model with fp16-overflowing weights"
...
def test_checkpoint_nonfinite_ema_resync():
    ...
    assert (
        trainer.last.exists()
    ), "no checkpoint saved when the EMA went non-finite on a finite model"
    model, _ = load_checkpoint(trainer.last)
    assert all(
        torch.isfinite(v).all()
        for v in model.state_dict().values()
...
def test_checkpoint_nonfinite_ema_and_model_sanitized():
    ...
    def poison_ema_and_model(trainer):
        ...
        if trainer.ema is not None:
            next(iter(trainer.ema.ema.parameters())).data.flatten()[0] = float("inf")
            next(iter(unwrap_model(trainer.model).parameters())).data.flatten()[0] = (
                float("nan")
            )
    ...
    assert (
        trainer.last.exists()
    ), "no checkpoint saved when a tensor went non-finite in both EMA and model"
    model, _ = load_checkpoint(trainer.last)
    assert all(
        torch.isfinite(v).all()
        for v in model.state_dict().values()
...
def test_train_reuses_loaded_checkpoint_model(monkeypatch, kwargs, uses_weights):
    ...
    monkeypatch.setattr(
        "ultralytics.engine.model.checks.check_pip_update_available", lambda: None
    )
    ...
    assert (
        captured["trainer"].model is original_model
    ), "Trainer model does not match original"
    assert (
        captured["cfg"] == original_model.yaml
    ), f"Config mismatch: {captured['cfg']} != {original_model.yaml}"
    assert captured["weights"] is (
        original_model if uses_weights else None
    ...
    class FakeTrainer:
        ...
        def train(self):
            ...
            self.validator = SimpleNamespace(
                metrics=SimpleNamespace(results_dict={"fitness": 1.0})
            )
...
@pytest.mark.parametrize(
    "pretrained,uses_weights", [(True, True), (False, False), (MODEL, True)]
)
def test_setup_model_respects_pretrained_arg_for_pt_models(
    monkeypatch, pretrained, uses_weights
):
    ...
    monkeypatch.setattr(
        ...
        lambda path: (checkpoint_model, {"checkpoint": True}),
    )
    ...
    assert captured["weights"] is (
        checkpoint_model if uses_weights else None
    ), "Unexpected weights loaded"
