...
if CUDA_IS_AVAILABLE:
    if IS_JETSON:
        DEVICES = [
            0
        ]  # NVIDIA Jetson only has one GPU and does not fully support pynvml library
    else:
        gpu_info = GPUInfo()
        gpu_info.print_status()
        autodevice_fraction = __import__("os").environ.get(
            "YOLO_AUTODEVICE_FRACTION_FREE", 0.3
        )
...
@pytest.mark.parametrize(
    ...
    [  # generate all combinations except for exclusion cases
        ...
        if not (
            ...
            or (task == "obb" and nms and (not TORCH_1_13 or IS_JETSON))
        )
    ],
)
def test_export_onnx_matrix(task, dynamic, batch, simplify, nms):
    ...
    YOLO(file)(
        [SOURCE] * batch, imgsz=64 if dynamic else 32, device=DEVICES[0]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        for task, dynamic, quantize, batch in product(
            ["detect"] if IS_JETSON else sorted(TASKS), [True], [8, 16], [2]
        )
    ],
)
def test_export_engine_matrix(task, dynamic, quantize, batch):
    ...
    if not is_trt11 and quantize == 8 and dynamic:
        ...
        pytest.skip(
            "INT8 + dynamic export requires explicit quantization, available on TensorRT 11+"
        )
    ...
    Path(file).unlink()  # cleanup
    if quantize == 8:
        Path(file).with_suffix(".cache").unlink(
            missing_ok=True
        )  # cleanup TensorRT 7-10 INT8 calibration cache
        Path(file).with_suffix(".int8.onnx").unlink(
            missing_ok=True
        )  # cleanup TensorRT 11 ModelOpt INT8 ONNX
    if quantize == 16:
        Path(file).with_suffix(".fp16.onnx").unlink(
            missing_ok=True
        )  # cleanup TensorRT 11 ModelOpt FP16 ONNX
...
def test_train():
    ...
    expected = parse_device(
        device
    )  # canonical torch indices, e.g. physical ids translate under external CVD
    visible = os.environ.get("CUDA_VISIBLE_DEVICES")
    results = YOLO(MODEL).train(
        data="coco8-grayscale.yaml", imgsz=64, epochs=1, device=DEVICES[0], batch=-1
    )
    model = YOLO(MODEL)
    results = model.train(
        data="coco8.yaml", imgsz=64, epochs=1, device=device, batch=15, compile=True
    )
    assert model.trainer.args.device == expected, "trained on wrong GPUs"
    assert model.trainer.device.index == int(
        expected.split(",")[0]
    ), "trained on wrong GPU"
    assert (
        os.environ.get("CUDA_VISIBLE_DEVICES") == visible
    ), "CUDA_VISIBLE_DEVICES must never be mutated"
    results = YOLO(MODEL).train(
        data="coco128.yaml", imgsz=64, epochs=1, device=device, batch=15, val=False
    )
...
@pytest.mark.skipif(
    not DEVICES or max(DEVICES) == 0,
    reason="requires an idle CUDA device with nonzero index",
)
@pytest.mark.skipif(IS_JETSON, reason="Edge devices not intended for training")
...
def test_autobatch():
    ...
    check_train_batch_size(
        YOLO(MODEL).model.to(f"cuda:{DEVICES[0]}"), imgsz=64, amp=True
    )
...
def test_utils_benchmarks(isolated_model):
    ...
    YOLO(isolated_model).export(
        format="engine", imgsz=32, dynamic=True, batch=1, device=DEVICES[0]
    )
    ProfileModels(
        [isolated_model],
        imgsz=32,
