...
def test_export(model: str, tmp_path: Path) -> None:
    ...
    for end2end in {False, True}:
        run(
            f"yolo export model={isolated} format=torchscript imgsz=32 end2end={end2end} max_det=100"
        )
...
def test_distill(task: str, data: str, student: str, teacher: Path) -> None:
    ...
    run(
        f"yolo train {task} model={student} distill_model={teacher} data={data} imgsz=32 epochs=1"
    )
...
def test_rtdetr(
    task: str = "detect",
    model: Path = WEIGHTS_DIR / "rtdetr-l.pt",
    data: str = "coco8.yaml",
) -> None:
    ...
    run(
        f"yolo train {task} model={model} data={data} --imgsz= 160 epochs =1, cache = disk"
    )


@pytest.mark.skipif(
    IS_RASPBERRYPI, reason="Edge devices not intended for heavy FastSAM tests"
)
@pytest.mark.skipif(
    checks.IS_PYTHON_3_12, reason="MobileSAM with CLIP is not supported in Python 3.12"
)
@pytest.mark.skipif(
...
def test_fastsam(
    task: str = "segment",
    model: str = WEIGHTS_DIR / "FastSAM-s.pt",
    data: str = "coco8-seg.yaml",
) -> None:
    ...
    for s in (source, Image.open(source)):
        everything_results = sam_model(
            s, device="cpu", retina_masks=True, imgsz=160, conf=0.4, iou=0.9
        )

        # Remove small regions
        _new_masks, _ = Predictor.remove_small_regions(
...
def test_train_gpu(task: str, model: str, data: str) -> None:
    ...
    run(
        f"yolo train {task} model={model} data={data} imgsz=32 epochs=1 device=0"
    )  # single GPU
    run(
        f"yolo train {task} model={model} data={data} imgsz=32 epochs=1 device=0,1"
    )  # multi GPU
