...
GITHUB_ICON = (
    '<i class="fa-brands fa-github" aria-hidden="true" style="margin-right:6px;"></i>'
)
...
def _with_reference_title(header_content: str, module_path: str) -> str:
    ...
    """
    if re.search(
        r"(?m)^title\s*:", header_content
    ):  # line-anchored: ignore `title:` inside a description
        return header_content
    title = f"{module_path.removeprefix(f'{PACKAGE_DIR.name}.')} API Reference"
    return header_content.replace("---\n", f"---\ntitle: {title}\n", 1)


def create_placeholder_markdown(
    py_filepath: Path, module_path: str, classes: list[str], functions: list[str]
) -> Path:
    """Create a minimal Markdown reference stub."""
    md_filepath = REFERENCE_DIR / py_filepath.relative_to(PACKAGE_DIR).with_suffix(
        ".md"
    )
    ...
    md_content = ["<br>\n\n"]
    md_content.extend(
        f"## ::: {module_path_dots}.{cls}\n\n<br><br><hr><br>\n\n" for cls in classes
    )
    md_content.extend(
        f"## ::: {module_path_dots}.{func}\n\n<br><br><hr><br>\n\n"
...
def collect_signature_parameters(
    args: ast.arguments, src: str, *, skip_self: bool = True
) -> list[ParameterDoc]:
...
def _parse_named_entries(lines: list[str]) -> list[ParameterDoc]:
    ...
    for block in _split_section_entries(lines):
        ...
        if match:
            ...
            entries.append(
                ParameterDoc(
                    name=name, type=type_hint, description=_normalize_text(description)
                )
            )
        else:
...
def _parse_returns(lines: list[str]) -> list[ReturnDoc]:
    ...
    for block in _split_section_entries(lines):
        ...
        if match:
            ...
            entries.append(
                ReturnDoc(type=cleaned_type, description=_normalize_text(desc.strip()))
            )
        else:
...
def parse_google_docstring(docstring: str | None) -> ParsedDocstring:
    ...
    for line in body:
        ...
        if (
            current != "methods"
        ):  # ignore "Methods:" sections; methods are rendered from AST
    ...
    return ParsedDocstring(
        ...
        references=[
            line.strip() for line in sections.get("references", []) if line.strip()
        ],
    )


def merge_docstrings(
    base: ParsedDocstring, extra: ParsedDocstring, ignore_summary: bool = True
) -> ParsedDocstring:
    ...
    if extra.description:
        base.description = "\n\n".join(
            filter(None, [base.description, extra.description])
        )
    _merge_unique(
        base.params, extra.params, lambda p: (p.name, p.type, p.description, p.default)
    )
    _merge_unique(
        base.attributes,
        extra.attributes,
        lambda p: (p.name, p.type, p.description, p.default),
    )
    ...
    _merge_unique(
        base.raises, extra.raises, lambda r: (r.name, r.type, r.description, r.default)
    )
...
def _collect_source_block(src: str, node: ast.AST, end_line: int | None = None) -> str:
    ...
    decorator_lines = [
        getattr(d, "lineno", node.lineno) for d in getattr(node, "decorator_list", [])
    ]
    start_line = (
        min([*decorator_lines, node.lineno]) if decorator_lines else node.lineno
    )
...
def parse_function(
    ...
    *,
    ...
    if (
        not _should_document(node.name, allow_private=allow_private)
        and not raw_docstring
    ):
        return None
    ...
    signature_params = collect_signature_parameters(
        node.args, src, skip_self=bool(parent)
    )
...
def parse_class(node: ast.ClassDef, module_path: str, src: str) -> DocItem:
    ...
    if init_node:
        ...
        signature_params = collect_signature_parameters(
            init_node.args, src, skip_self=True
        )

    bases = [_get_source(src, b) for b in node.bases] if node.bases else []
    signature_node = init_node or node
    class_signature = format_signature(
        signature_node, src, is_class=True, display_name=node.name
    )
    ...
    for child in node.body:
        if (
            ...
            and child is not init_node
        ):
            method_doc = parse_function(
                child, module_path, src, parent=f"{module_path}.{node.name}"
            )
            if method_doc:
    ...
    return DocItem(
        ...
        children=methods,
        module_path=module_path,
        source=_collect_source_block(
            src, node, end_line=init_node.end_lineno if init_node else node.lineno
        ),
    )
...
def parse_module(py_filepath: Path) -> DocumentedModule | None:
    ...
    return DocumentedModule(
        path=py_filepath, module_path=module_path, classes=classes, functions=functions
    )
...
def _render_table(
    headers: list[str], rows: list[list[str]], level: int, title: str | None = None
) -> str:
...
def _merge_params(
    doc_params: list[ParameterDoc], signature_params: list[ParameterDoc]
) -> list[ParameterDoc]:
...
def contribution_admonition(
    pretty: str, url: str, *, kind: str = "note", title: str | None = None
) -> str:
...
def render_docstring(
    ...
    if merged_params:
        ...
        for p in merged_params:
            default_val = (
                f"`{p.default}`" if p.default not in (None, "") else "*required*"
            )
            rows.append(
                [
                    f"`{p.name}`",
        ...
        table = _render_table(
            ["Name", "Type", "Description", "Default"], rows, level, title=None
        )
    ...
    if doc.attributes:
        ...
        for a in doc.attributes:
            rows.append(
                [
                    f"`{a.name}`",
                    f"`{a.type}`" if a.type else "",
                    a.description.strip() if a.description else "",
                ]
            )
    ...
    if doc.references:
        links = "\n".join(
            ref if ref.startswith("- ") else f"- {ref}" for ref in doc.references
        )
        ...
        for child in cls.children:
            if child.kind == "property":
                property_links.append(
                    f"- [`{cls.name}.{child.name}`](#{item_anchor(child)})"
                )
        for child in cls.children:
...
def render_item(
    item: DocItem, module_url: str, module_path: str, level: int = 2
) -> str:
    """Render a class, function, or method to Markdown."""
    anchor = item_anchor(item)
    title_prefix = item.kind.capitalize()
    anchor_id = anchor.replace(
        "_", r"\_"
    )  # escape underscores so attr_list keeps them in the id
    heading = (
        f"{'#' * level} {title_prefix} `{display_qualname(item)}` {{#{anchor_id}}}"
    )
    ...
    if item.kind == "class":
        ...
        if item.children:
            ...
            for child in props + methods:
                summary = child.doc.summary or (
                    ...
                    if child.doc.description
                    else ""
                )
                rows.append(
                    [f"[`{child.name}`](#{item_anchor(child)})", summary.strip()]
                )
            if rows:
                table = _render_table(
                    ["Name", "Description"], rows, level + 1, title=None
                )
        ...
        parts.append(
            render_docstring(
                item.doc, level + 1, signature_params=item.signature_params
            )
        )
...
def create_markdown(module: DocumentedModule) -> Path:
    ...
    md_filepath = REFERENCE_DIR / module.path.relative_to(PACKAGE_DIR).with_suffix(
        ".md"
    )
    exists = md_filepath.exists()
    ...
    md_filepath.write_text(
        header_content + title_content + render_module_markdown(module)
    )

    if not exists:
        subprocess.run(
            ["git", "add", "-f", str(md_filepath)], check=True, cwd=REPO_ROOT
        )
...
def sort_nested_dict(d: dict) -> dict:
    ...
    return {
        k: sort_nested_dict(v) if isinstance(v, dict) else v
...
def create_nav_menu_yaml(nav_items: list[str]) -> str:
    ...
    LOGGER.info(
        f"Scan complete, generated reference section with {len(reference_yaml.splitlines())} lines"
    )
...
def update_mkdocs_file(reference_yaml: str) -> None:
    ...
    inner_lines = [
        line for line in reference_yaml.splitlines() if line.strip() != "- reference:"
    ]
    ...
    if ref_match:
        ...
        LOGGER.info(
            f"Found existing top-level Reference section ({len(ref_section)} chars)"
        )
        ...
        if len(existing_paths) == len(new_paths) and set(existing_paths) == set(
            new_paths
        ):
            LOGGER.info(
                f"No changes detected in document paths ({len(existing_paths)} items). Skipping update."
            )
            return

        LOGGER.info(
            f"Changes detected: {len(new_paths)} document paths vs {len(existing_paths)} existing"
        )
        ...
        try:
            ...
            LOGGER.warning(
                "prettier not found (install Node.js or run 'npm i -g prettier'), skipping YAML formatting"
            )
        ...
        new_content = mkdocs_content.replace(
            help_section, f"{new_ref_section}{help_section}"
        )
        MKDOCS_YAML.write_text(new_content)
...
def _finalize_reference(
    nav_items: list[str], update_nav: bool, created: int, created_label: str
) -> list[str]:
...
def build_reference_placeholders(update_nav: bool = True) -> list[str]:
    ...
    for py_filepath in TQDM(
        list(PACKAGE_DIR.rglob("*.py")), desc="Building reference stubs", unit="file"
    ):
        ...
        md_rel = create_placeholder_markdown(
            py_filepath, module_path, classes, functions
        )
...
def build_reference_docs(update_nav: bool = False) -> list[str]:
    ...
    for py_filepath in TQDM(list(PACKAGE_DIR.rglob("*.py")), desc=desc, unit="file"):
        md_target = REFERENCE_DIR / py_filepath.relative_to(PACKAGE_DIR).with_suffix(
            ".md"
        )
    ...
    if _missing_type_warnings:
        LOGGER.warning(
            f"{len(_missing_type_warnings)} functions/methods have parameters missing type annotations:"
        )
...
def build_reference_for(
    ...
    try:
        PACKAGE_DIR, REFERENCE_DIR, GITHUB_REPO = (
            package_dir,
            reference_dir,
            github_repo,
        )
