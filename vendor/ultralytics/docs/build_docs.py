...
os.environ["JUPYTER_PLATFORM_DIRS"] = (
...
def update_docs_html():
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        pbar = TQDM(
            executor.map(_process_html_file, html_files),
...
def _process_html_file(html_file: Path) -> bool:
    ...
    if rel_path == "404.html":
        new_content = re.sub(
            r"<title>.*?</title>",
            "<title>Ultralytics Docs - Not Found</title>",
            content,
        )
...
def update_docs_soup(
    content: str, html_file: Path | None = None, max_title_length: int = 70
) -> str:
    ...
    needs_title_trim = bool(
        title_match
        and len(title_match.group(1)) > max_title_length
        and "-" in title_match.group(1)
    )
    needs_link_conversion = ("<p" in content or "<li" in content) and bool(
        LINK_PATTERN.search(content)
    )
    ...
    if not (
        needs_title_trim
        or needs_link_conversion
        or needs_codelineno_cleanup
        or needs_kind_highlight
    ):
        return content
        ...
        for a in soup.select('a[href^="#__codelineno-"], a[id^="__codelineno-"]'):
            if a.string:  # If the a tag has text (the line number)
                ...
                if (
                    ...
                    and "normal" in a.parent.get("class", [])
                ):
                    del a.parent["class"]
    ...
    def highlight_labels(nodes):
        ...
        for node in nodes:
            ...
            if kind not in DOC_KIND_LABELS:
                continue
            span = soup.new_tag(
                "span", attrs={"class": f"doc-kind doc-kind-{kind.lower()}"}
            )
    ...
    if "reference" in rel_path:
        ...
        highlight_labels(
            soup.select(
                "nav.md-nav--secondary .md-ellipsis, nav.md-nav__list .md-ellipsis"
            )
        )
    ...
    if "reference" in rel_path:
        for ellipsis in soup.select("nav.md-nav--secondary .md-ellipsis"):
            ...
            text = (
                str(kind.next_sibling).strip()
                if kind and kind.next_sibling
                else ellipsis.get_text(strip=True)
            )
            if "." not in text:
                continue
            ellipsis.clear()
...
HTML_PRESERVE = re.compile(
    r"<(pre|code|textarea|script)[^>]*>[\s\S]*?</\1>", re.IGNORECASE
)
...
def minify_files(html: bool = True, css: bool = True, js: bool = True):
    ...
    for ext, minifier in {
        "html": (
            (
                lambda x: minify(
                    x, keep_closing_tags=True, minify_css=True, minify_js=True
                )
            )
            if html
            else None
        ...
        if not files:
            continue
        pbar = TQDM(
            files, desc=f"Minifying {ext.upper()} - reduced 0.00% (0.00 KB saved)"
        )
        for f in pbar:
            ...
            out = (
                minifier(content)
                if minifier
                else remove_comments_and_empty_lines(content, ext)
            )
            orig += len(content)
            minified += len(out)
            ...
            pbar.set_description(
                f"Minifying {ext.upper()} - reduced {pct:.2f}% ({saved / 1024:.2f} KB saved)"
            )
...
def render_jinja_macros() -> None:
    ...
    def indent_filter(
        value: str, width: int = 4, first: bool = False, blank: bool = False
    ) -> str:
    ...
    for md_file in pbar:
        ...
        try:
            rendered = env.render_str(
                markdown_content, name=str(md_file.relative_to(DOCS)), **context
            )
        except Exception as e:
        ...
        pbar.set_description(
            f"MiniJinja: {macros_total} macros, {files_with_macros} pages"
        )
...
def main():
    ...
    if not shutil.which("zensical"):
        raise SystemExit(
            "zensical is not installed. Install it with: pip install -e '.[dev]'"
        )
    ...
    try:
        ...
        subprocess.run(
            ["zensical", "build", "-f", str(DOCS.parent / "mkdocs.yml"), "--strict"],
            check=True,
        )
        ...
        if postprocess_site:
            ...
            LOGGER.warning(
                "postprocess_site not available; skipping docs postprocessing"
            )
        ...
        if sitemap.exists():
            ...
            if missing := (all_pages - in_sitemap):
                ...
                sitemap.write_text(
                    content.replace("</urlset>", f"{entries}\n</urlset>")
                )
            LOGGER.info(
        ...
        if (MACOS or LINUX) and not os.getenv("GITHUB_ACTIONS"):
            ...
            try:
                ...
                if "Address already in use" in str(e):
                    LOGGER.info(
                        "Port 8000 in use; skipping auto-serve. Serve manually if needed."
                    )
                else:
                    LOGGER.info(f"\n❌ Server failed: {e}")
        else:
