...
def extend_line_from_edge(
    mid_x: int, mid_y: int, direction: str, img_shape: tuple[int, int, int]
) -> tuple[int, int]:
...
def draw_tracking_scope(im, bbox: tuple, color: tuple) -> None:
    ...
    cv2.line(
        im, mid_bottom, extend_line_from_edge(*mid_bottom, "down", im.shape), color, 2
    )
    cv2.line(im, mid_left, extend_line_from_edge(*mid_left, "left", im.shape), color, 2)
    cv2.line(
        im, mid_right, extend_line_from_edge(*mid_right, "right", im.shape), color, 2
    )
...
while cap.isOpened():
    ...
    results = model.track(
        im, conf=conf, iou=iou, max_det=max_det, tracker=tracker, **track_args
    )
    ...
    for track in detections:
        ...
        label = f"{class_name} ID {track_id}" + (
            f" ({conf_score:.2f})" if show_conf else ""
        )
        ...
        if track_id == selected_object_id:
            ...
            annotator.box_label(
                [x1, y1, x2, y2], label=f"ACTIVE: TRACK {track_id}", color=color
            )
        else:
    ...
    if show_fps:
        ...
        cv2.rectangle(
            im, (10 - 5, 25 - th - 5), (10 + tw + 5, 25 + bl), (255, 255, 255), -1
        )
