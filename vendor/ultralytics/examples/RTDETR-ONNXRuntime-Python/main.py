...
class RTDETR:
    ...
    def __init__(
        ...
        self.session = ort.InferenceSession(
            model_path, providers=providers or available
        )
        ...
        self.color_palette: np.ndarray = np.random.uniform(
    ...
    def postprocess(self, model_output: list[np.ndarray]) -> np.ndarray:
        ...
        indices = cv2.dnn.NMSBoxes(
            xywh_boxes, scores.tolist(), self.conf_thres, self.iou_thres
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model",
        type=str,
        default="rtdetr-l.onnx",
        help="Path to the ONNX model file.",
    )
    parser.add_argument(
        "--img", type=str, default="bus.jpg", help="Path to the input image."
    )
    parser.add_argument(
        "--conf-thres",
        type=float,
