...
def build_pipeline(
    model_path: str, conf: float = 0.25, tracker_algo: str | None = "tracktrack"
):
...
def draw_pose(
    image: np.ndarray, detections: np.ndarray, conf: float = 0.25
) -> np.ndarray:
    ...
    for det in detections:
        ...
        kpts = det[6:].reshape(
            -1, 3
        )  # 17 x (x, y, confidence); x/y normalized, conf raw
...
def main():
    ...
    parser.add_argument(
        "--source", type=str, default="0", help="Image, video path, or camera index"
    )
    ...
    while True:
        ...
        annotated = (
            draw_tracked_poses(frame, results)
            if use_tracking
        ...
        if args.no_display:
            ...
            if frame_count % 100 == 0:
                print(
                    f"  frame {frame_count}: {len(results)} {'tracks' if use_tracking else 'detections'}"
                )
        else:
            title = (
                "Ultralytics YOLO26 Pose Tracking"
                if use_tracking
