...
def build_pipeline(model_path: str, conf: float = 0.25, iou: float = 0.45):
    ...
    return op.seq(
        ...
        op.decode_segmentation(
            algo="yolo11", num_classes=80, num_mask_coeffs=32, confidence_threshold=conf
        ),
        op.par(
...
def main(args):
    ...
    with display.App(renderer=renderer) as visualizer:
        vis = visualizer.create_window(
            "YOLO11 Segmentation", (800, 500), expose_surface=expose_surface
        )
        ...
        source_arg = (
            f"/dev/video{args.source}" if args.source.isdigit() else args.source
        )
        ...
        with cv.create_source(source_arg, backend=backend) as source:
            ...
            for frame_count, (img, segments) in enumerate(
                pipeline.stream(source), start=1
            ):
                if vis.is_closed:
                    break
                vis(img, segments)
                ...
                if args.no_display and frame_count % 100 == 0:
                    counts = Counter(
                        getattr(s.class_id, "name", str(s.class_id)) for s in segments
                    )
                    summary = (
        ...
        is_image = (
            not args.source.isdigit()
            and Path(args.source).suffix.lower() in IMAGE_SUFFIXES
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="YOLO11 Instance Segmentation -- Axelera Voyager SDK"
    )
    parser.add_argument(
        "--model", type=str, required=True, help="Path to compiled .axm model"
    )
    parser.add_argument(
        "--source", type=str, required=True, help="Image, video path, or camera index"
    )
    parser.add_argument("--conf", type=float, default=0.25, help="Confidence threshold")
    parser.add_argument("--iou", type=float, default=0.45, help="NMS IoU threshold")
    parser.add_argument(
        "--no-display", action="store_true", help="Disable GUI window (headless mode)"
    )
    parser.add_argument(
        "--output",
        type=str,
        default="",
        help="Save the rendered video to this path (requires a display, not headless)",
    )
    args = parser.parse_args()
    if args.output and args.no_display:
        parser.error(
            "--output requires the display renderer and cannot be combined with --no-display"
        )
    main(args)
