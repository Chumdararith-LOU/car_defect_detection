...
class YOLOv8Seg:
    ...
    def postprocess(
        self, img: np.ndarray, prep_img: np.ndarray, outs: list
    ) -> list[Results]:
        ...
        preds = nms.non_max_suppression(
            preds, self.conf, self.iou, nc=len(self.classes)
        )
        ...
        for i, pred in enumerate(preds):
            ...
            results.append(
                Results(
                    img, path="", names=self.classes, boxes=pred[:, :6], masks=masks
                )
            )
