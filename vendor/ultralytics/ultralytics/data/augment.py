...
class Mosaic(BaseMixTransform):
    ...
    def apply_image(
        self, labels: dict[str, Any], params: dict[str, Any] | None = None
    ) -> dict[str, Any]:
        ...
        if self.n == 4:
            img4 = np.full(
                (self.imgsz * 2, self.imgsz * 2, labels["img"].shape[2]),
                114,
                dtype=np.uint8,
            )
            for item in layout:
            ...
            img9 = np.full(
                (self.imgsz * 3, self.imgsz * 3, labels["img"].shape[2]),
                114,
                dtype=np.uint8,
            )
            for item in layout:
    ...
    def apply_instances(
        ...
        for item in layout:
            ...
            labels_patch = self._update_labels(
                item["labels_patch"], padw, padh, item.get("img_shape")
            )
    ...
    @staticmethod
    def _update_labels(
        labels, padw: int, padh: int, img_shape: tuple[int, int] | None = None
    ) -> dict[str, Any]:
...
class RandomPerspective(BaseTransform):
    ...
    def _compute_affine_matrix(
        self, img: np.ndarray, size: tuple[int, int]
    ...
    def get_params(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        img = labels["img"]
        if (
            rect_shape := labels.get("rect_shape")
        ) is not None:  # rect has higher priority
            size = (
                int(rect_shape[1]),
                int(rect_shape[0]),
            )  # rect mode batch shape (h, w) to (w, h)
        else:
            size = (
                (img.shape[1], img.shape[0]) if self.size is None else self.size
            )  # w, h
    ...
    def apply_image(
        ...
        M = params["M"]
        size = params["size"]
        if (size[0] != img.shape[1] or size[1] != img.shape[0]) or (
            M != np.eye(3)
        ).any():  # image changed
            if self.perspective:
                img = cv2.warpPerspective(
                    img, M, dsize=size, borderValue=(114, 114, 114)
                )
            else:  # affine
                img = cv2.warpAffine(
                    img, M[:2], dsize=size, borderValue=(114, 114, 114)
                )
            if img.ndim == 2:
                img = img[..., None]
        labels["img"] = img
    ...
    def apply_keypoints(
        self, keypoints: np.ndarray, M: np.ndarray, size: tuple[int, int]
    ) -> np.ndarray:
    ...
    def apply_semantic(
        ...
        M = params["M"]
        size = params["size"]
        if (size[0] != mask.shape[1] or size[1] != mask.shape[0]) or (
            M != np.eye(3)
        ).any():
            if self.perspective:
                mask = cv2.warpPerspective(
                    mask, M, dsize=size, flags=cv2.INTER_NEAREST, borderValue=255
                )
            else:
                mask = cv2.warpAffine(
                    mask, M[:2], dsize=size, flags=cv2.INTER_NEAREST, borderValue=255
                )
        labels["semantic_mask"] = mask
        return labels
...
class RandomFlip(BaseTransform):
    ...
    def apply_image(
        self, labels: dict[str, Any], params: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        if params["flip"]:
            ...
            if params["flip_idx"] is not None and instances.keypoints is not None:
                instances.keypoints = np.ascontiguousarray(
                    instances.keypoints[:, params["flip_idx"], :]
                )
    ...
    def apply_semantic(
        ...
        if params["flip"]:
            if params["direction"] == "vertical":
                labels["semantic_mask"] = np.ascontiguousarray(
                    np.flipud(labels["semantic_mask"])
                )
            elif params["direction"] == "horizontal":
                labels["semantic_mask"] = np.ascontiguousarray(
                    np.fliplr(labels["semantic_mask"])
                )
        return labels
...
class CopyPaste(BaseMixTransform):
    ...
    def apply_instances(
        ...
        for j in selected:
            cls = np.concatenate(
                (cls, (labels2_cls if labels2_cls is not None else cls)[[j]]), axis=0
            )
    ...
    def apply_semantic(
        ...
        source = (
            labels.get("mix_labels", [{}])[0].get("semantic_mask")
            if self.mode == "mixup"
            else cv2.flip(mask, 1)
        )
        if source is None:
            return labels
...
class Format(BaseTransform):
    ...
    def apply_instances(
        ...
        ), "instances are required for Format.apply_instances"
        h = params.get("h", 0)
        w = params.get("w", 0)
        nl = params.get("nl", 0)
...
def v8_transforms(dataset, imgsz: int, hyp: IterableSimpleNamespace):
    ...
    if hyp.copy_paste_mode == "flip":
        pre_transform.insert(
            1, CopyPaste(dataset, p=hyp.copy_paste, mode=hyp.copy_paste_mode)
        )
    else:
