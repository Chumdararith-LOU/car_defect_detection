...
class YOLODataset(BaseDataset):
    ...
    def cache_labels(self, path: Path = Path("./labels.cache")) -> dict:
        ...
        if nf == 0:
            if (
                self.augment
            ):  # training requires labels; unlabeled val splits (e.g. COCO test-dev) only warn
    ...
    def collate_fn(batch: list[dict]) -> dict:
        ...
        if "batch_idx" in new_batch:
            ...
            for i in range(len(new_batch["batch_idx"])):
                new_batch["batch_idx"][
                    i
                ] += i  # add target image index for build_targets()
...
class SemanticDataset(YOLODataset):
    ...
    def update_labels(self, include_class: list[int] | None) -> None:
        ...
        self.include_class = (
            ...
            else np.asarray(include_class, dtype=np.int32).reshape(-1)
        )
    ...
    def get_labels(self):
        ...
        """
        self.mask_files = img2label_paths(
            self.im_files, label_dir=self.data.get("masks_dir", "masks"), suffix=".png"
        )
    ...
    def load_mask(
        self, index: int, image_shape: tuple[int, int] | None = None
    ) -> np.ndarray:
        ...
        if mask is None:
            raise FileNotFoundError(
                f"Semantic mask not found or unreadable: {mask_file}"
            )
        ...
        if int(self.data.get("nc", 0)) == 1:
            with Image.open(mask_file) as im:
                if (
                    im.mode == "1"
                ):  # cv2 expands 1-bit PNGs to {0, 255}; map only true 1-bit foreground to 1.
                    mask[mask == 255] = 1
    ...
    def build_transforms(self, hyp=None):
        ...
        transforms[-1] = (
            SemanticFormat()
        )  # replace the last transform with SemanticFormat
        return transforms
    ...
    def get_image_and_label(self, index):
        ...
        if (
            self.include_class is not None
        ):  # keep only selected classes; remap the rest to the ignore label
...
class PolygonSemanticDataset(SemanticDataset, YOLODataset):
    ...
    def load_mask(
        ...
        fg = inst > 0
        if (
            int(self.data.get("nc", 0)) == 1
        ):  # binary: fg=1 regardless of label cls value
            out[fg] = 1
        else:
...
class ClassificationDataset:
    ...
    def __getitem__(self, i: int) -> dict:
        ...
        if self.cache_ram:
            ...
            im = self.img_cache[pos : pos + h * w * c].reshape(
                h, w, c
            )  # zero-copy view
