...
def convert_to_multispectral(
    ...
    if path.is_dir():
        ...
        seg = np.clip(
            np.searchsorted(xp, target_wavelengths) - 1, 0, len(xp) - 2
        )  # segment per target
        w = (target_wavelengths - xp[seg]) / (
            xp[seg + 1] - xp[seg]
        )  # weights (<0 or >1 -> extrapolation)
        img = img[..., order]
...
async def convert_ndjson_to_yolo(
    ...
    if not is_classification:
        ...
        if "val" not in splits:
            ...
            random.Random(0).shuffle(
                train_records
            )  # local RNG to avoid mutating global training seed
    ...
    if _reuse:
        yaml_path.unlink(
            missing_ok=True
        )  # Invalidate hash before destructive ops (crash safety)
        if not is_classification:
                ...
                if not image_path.exists() and (http_url := record.get("url")):
                    ...
                    for attempt in range(3):
                        ...
                        try:
                            ...
                            Exception
                        ) as e:  # OSError, disk full, permissions — not transient, don't retry
