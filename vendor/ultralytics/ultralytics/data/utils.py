...
def convert_ndjson_to_yolo_if_needed(data: str | Path) -> str | Path:
    ...
    data = normalize_platform_uri(
        data
    )  # accept Platform web URLs (https://platform.ultralytics.com/.../datasets/...)
    data_str = str(data)
...
def check_det_dataset(dataset: str, autodownload: bool = True) -> dict[str, Any]:
    ...
    if (
        "://" not in dataset
        and not Path(dataset).exists()
        and Path(dataset).suffix not in {".yaml", ".yml"}
    ):
        ...
        dataset = next(
            (
                f"{dataset}{x}"
                for x in (".yaml", ".yml")
                if check_file(f"{dataset}{x}", hard=False)
            ),
            dataset,
        )
    ...
    if "nc" in data and not isinstance(data["nc"], int):
        try:
            ...
            raise SyntaxError(
                emojis(f"{dataset} 'nc: {data['nc']}' must be an integer ❌.")
            )
    ...
    def process_images(self) -> Path:
        ...
        for split in "train", "val", "test":
            ...
            if self.task == "classify":
                ...
                dataset = YOLODataset(
                    img_path=split_path, data=self.data, task=self.task
                )
                im_files = dataset.im_files
            with ThreadPool(NUM_THREADS) as pool:
                for _ in TQDM(
                    pool.imap(self._hub_ops, im_files),
                    total=len(im_files),
                    desc=f"{split} images",
                ):
                    pass
