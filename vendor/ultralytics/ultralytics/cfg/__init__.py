...
QUANTIZE_VALID_VALUES = (
    "8, 16, 32, 'int8', 'fp16', 'fp32', 'w8a8', 'w16a16', 'w8a16', or 'w8a32'"
)
    ...
    for k, v in cfg.items():
        if v is not None:  # None values may be from optional args
            if k in CFG_FLOAT_KEYS and not isinstance(v, FLOAT_OR_INT):
                ...
                k == "quantize"
            ):  # canonicalize 8/16/32 or w-notation to a scheme (unset stays None for FP32)
...
def _handle_deprecation(custom: dict) -> dict:
    ...
    if (int8 is not None or half is not None) and "quantize" not in custom:
        int8_on = int8 is not None and str(int8).strip().lower() not in {
            "none",
            "false",
            "0",
        }
        half_on = half is not None and str(half).strip().lower() not in {
            "none",
            "false",
            "0",
        }
        custom["quantize"] = (
            8 if int8_on else 16 if half_on else None
        )  # False/0 clears precision back to FP32
...
def handle_yolo_solutions(args: list[str]) -> None:
    ...
    if solution_name == "inference":
        ...
        if solution_name != "crop":
            ...
            if (
                solution_name == "analytics"
            ):  # analytical graphs follow fixed shape for output i.e w=1280, h=720
                w, h = 1280, 720
            ...
            )  # create the output directory i.e. runs/solutions/exp
            vw = cv2.VideoWriter(
...
def entrypoint(debug: str = "") -> None:
    ...
    special = {
        ...
        "help": lambda: LOGGER.info(
            CLI_HELP_MSG
        ),  # help below hub for -h flag precedence
    }
    full_args_dict = {
        **DEFAULT_CFG_DICT,
    ...
    for a in merge_equals_args(args):  # merge spaces around '=' sign
        ...
        if "=" in a:
            ...
            overrides[a] = (
                True  # deprecated bare precision flags, forwarded to quantize by _handle_deprecation
            )
    ...
    if "rtdetr" in stem:  # guess architecture
        ...
        if "yoloe" in stem or "world" in stem:
            ...
            if cls_list is not None and isinstance(cls_list, str):
                model.set_classes(
                    [c.strip() for c in cls_list.split(",")]
                )  # "person, bus" -> ['person', 'bus']
    # Task Update
    if task != model.task:
        if task:
            LOGGER.warning(
