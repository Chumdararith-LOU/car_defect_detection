    ...
    def set_classes(
        ...
        if sorted(list(self.model.names.values())) != sorted(classes):
            if embeddings is None:
                embeddings = self.get_text_pe(
                    classes
                )  # generate text embeddings if not provided
        ...
        **kwargs,
        ...
        if len(visual_prompts):
            ...
            if type(self.predictor) is not predictor:
                ...
                self.predictor = predictor(
                    overrides={
                        ...
                        "verbose": kwargs.get(
                            "verbose",
                            self.overrides.get("verbose", refer_image is None),
                        ),
                        "batch": 1,
                        "device": args.device,
            ...
            self.predictor.setup_model(
                model=self.model, verbose=self.predictor.args.verbose
            )
