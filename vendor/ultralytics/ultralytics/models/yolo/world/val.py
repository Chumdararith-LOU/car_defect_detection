...
class WorldValidator(DetectionValidator):
    ...
    def __call__(self, trainer=None, model=None):
        ...
        if (
            trainer is None
        ):  # standalone val; training sets classes via on_pretrain_routine_end callback
            ...
            self.args.data = convert_ndjson_to_yolo_if_needed(
                self.args.data
            )  # match BaseValidator dataset handling
            names = [
                name.split("/", 1)[0]
                for name in check_det_dataset(self.args.data)["names"].values()
            ]
            current = (
            ...
            if (
                list(current) != names
            ):  # regenerate prompts only if class order differs from dataset
                state = (
                    model.names,
                    model.txt_feats,
                    model.model[-1].nc,
                )  # restore after to avoid leak to caller
                model.set_classes(names, cache_clip_model=False)
                model.names = dict(
                    enumerate(names)
                )  # set_classes updates embeddings/nc but not names
                try:
