...
class SemanticSegmentationTrainer(DetectionTrainer):
        ...
        for idx in indices:
            ...
            try:
                mask = dataset.load_mask(
                    idx, image_shape=tuple(shape) if shape is not None else None
                )
            except Exception:
                continue
    ...
    def compute_class_weights(self, class_counts):
        ...
        p = class_counts / max(
            class_counts.sum(), 1.0
        )  # pixel frequency, bounded for rare classes unlike detection
    ...
    def plot_training_labels(self):
        ...
        bars = ax.bar(
            range(nc),
            pixel_counts,
            color=[list(c / 255.0 for c in colors(i, False)) for i in range(nc)],
        )
        ax.set_xlabel("Class")
