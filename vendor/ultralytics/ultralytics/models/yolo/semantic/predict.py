...
class SemanticSegmentationPredictor(BasePredictor):
    ...
    def _class_map_dtype(num_classes: int) -> torch.dtype:
        ...
        return (
            torch.uint8
            if num_classes <= 256
            else torch.int16 if num_classes <= 32768 else torch.int32
        )
    ...
    def postprocess(self, preds, img, orig_imgs):
        ...
        for i, (pred, orig_img) in enumerate(zip(preds, orig_imgs)):
            img_path = (
                self.batch[0][i] if isinstance(self.batch[0], list) else self.batch[0]
            )
            class_map_input = (
                pred.ndim == 2
            )  # exports with in-graph ArgMax emit a [H, W] class map directly
            ...
            if class_map_input:
                ...
                class_map = ops.scale_masks(pred, orig_img.shape[:2], mode="nearest")[
                    0, 0
                ]
                class_map = class_map.to(
                    self._class_map_dtype(int(class_map.max().item()) + 1)
                )
            else:
                ...
                class_map = (
                    pred.argmax(0).to(dtype)
                    if pred.shape[0] > 1
                    else pred.gt(0).squeeze(0).to(dtype)
                )
            if (
                classes is not None
            ):  # keep only selected classes, mark the rest as ignore
