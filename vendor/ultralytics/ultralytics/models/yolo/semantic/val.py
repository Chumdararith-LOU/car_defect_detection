...
class SemanticSegmentationValidator(DetectionValidator):
    ...
    def init_metrics(self, model):
        ...
        self.image_shapes = {
            lb["im_file"]: tuple(lb["shape"])
            for lb in labels
            if "im_file" in lb and "shape" in lb
        }
        self.results_dir = None
        ...
        if (
            cm_nc == 2 and len(self.names) == 1
        ):  # binary segmentation, expand to include background
            cm_names = {0: "background", 1: next(iter(self.names.values()))}
        else:
            base = list(self.names.values()) + [
                str(i) for i in range(len(self.names), cm_nc)
            ]
    ...
    def preprocess(self, batch):
        ...
        batch["semantic_mask"] = batch["semantic_mask"].to(
    ...
    def postprocess(self, preds):
        ...
        if preds.ndim == 3:
            ...
            if tuple(preds.shape[-2:]) != self._semantic_target_shape:
                preds = F.interpolate(
                    preds[:, None].float(),
                    size=self._semantic_target_shape,
                    mode="nearest",
                )[:, 0]
            return preds.to(torch.int32)
        pred_hw = preds.shape[2:]
        if (
            pred_hw[0] != self._semantic_target_shape[0]
            or pred_hw[1] != self._semantic_target_shape[1]
        ):
            preds = F.interpolate(
                preds,
                size=self._semantic_target_shape,
        ...
        return (
            preds.argmax(dim=1).to(torch.int32)
            if self.nc > 1
            else preds.gt(0).squeeze(1).to(torch.int32)
        )
    ...
    def gather_stats(self):
        ...
        if self.metrics.matrix is None:
            cm_nc = self.metrics.cm_nc
            self.metrics.matrix = torch.zeros(
                (cm_nc, cm_nc), device=self.device, dtype=torch.float32
            )
    ...
    def get_stats(self):
        ...
        if self.metrics.matrix is not None:
            ...
            self.confusion_matrix.matrix = (
                self.metrics.matrix.detach().cpu().numpy().T.astype(float)
            )
