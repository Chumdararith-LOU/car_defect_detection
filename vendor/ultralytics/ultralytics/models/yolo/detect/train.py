...
class DetectionTrainer(BaseTrainer):
        ...
        if self.args.multi_scale > 0.0:
            ...
            sz = (
                random.randrange(
                    ...
                    int(
                        self.args.imgsz * (1.0 + self.args.multi_scale) + self.stride
                    ),  # max imgsz
                )
                // self.stride
                * self.stride
    ...
    def get_class_counts(self):
        ...
        return np.bincount(classes.astype(int), minlength=self.data["nc"]).astype(
            np.float32
        )
    ...
    def set_class_weights(self):
        ...
        if (
            not class_counts.any()
        ):  # nothing counted (e.g. missing/unreadable masks); keep default weights
            return
        ...
        if hasattr(unwrap_model(model), "student_model"):
            model = unwrap_model(
                model
            ).student_model  # distillation: the student model builds the loss criterion
    ...
    def auto_batch(self):
        ...
        max_num_obj = (
            max(len(label["cls"]) for label in train_dataset.labels) * 4
