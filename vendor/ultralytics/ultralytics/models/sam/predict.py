...
class Predictor(BasePredictor):
    ...
    def inference_features(
        ...
        if pred_masks.shape[0] == 0:
            pred_masks, pred_bboxes = None, torch.zeros(
                (0, 6), device=pred_masks.device
            )
        else:
...
class SAM2VideoPredictor(SAM2Predictor):
    ...
    def __init__(self, cfg=DEFAULT_CFG, overrides=None, _callbacks: dict | None = None):
        ...
        self.clear_non_cond_mem = (
            True  # Whether to clear non-conditioning memory periodically
        )
    ...
    def add_new_prompts(
        ...
        return pred_masks.flatten(0, 1), torch.ones(
            1, dtype=pred_masks.dtype, device=pred_masks.device
        )
...
class SAM3SemanticPredictor(SAM3Predictor):
    ...
    def pre_transform(self, im):
        ...
        assert len(im) == 1, "SAM model does not currently support batched inference"
        letterbox = LetterBox(
            self.imgsz, auto=False, center=False, scale_fill=True
        )  # hardcode here for sam3
        return [letterbox(image=x) for x in im]
    ...
    def _inference_features(
        ...
        if bboxes is not None:
            ...
            if text is None:
                text = [
                    "visual"
                ]  # bboxes needs this `visual` text prompt if no text passed
...
class SAM3VideoSemanticPredictor(SAM3SemanticPredictor):
    ...
    def _get_objects_to_suppress_based_on_most_recently_occluded(
        ...
        iou = mask_iou(
            binary_low_res_masks.flatten(1), binary_low_res_masks.flatten(1)
        )  # [N,N]
