...
def export_formats():
    ...
    x = [
        ...
        [
            ...
            False,
            False,
            ["batch", "name", "quantize", "fraction", "data"],
            "base",
        ],
        [
            "LiteRT",
            ...
            True,
            False,
            ["batch", "quantize", "data", "fraction"],
            "litert",
        ],
    ]
    return dict(
        zip(["Format", "Argument", "Suffix", "CPU", "GPU", "Arguments", "Env"], zip(*x))
    )
...
EXPORT_ENVS = {
    ...
    "isolated-axelera": {
        ...
        "smoke": [
            "yolo export format=axelera model=yolo26n.pt imgsz=64 data=coco8.yaml"
        ],
    },
    "isolated-deepx": {
...
FP16_FORMATS = frozenset(
    {"torchscript", "onnx", "openvino", "engine", "coreml", "mnn", "ncnn", "rknn"}
)
...
W8A32_FORMATS = frozenset(
    {"litert"}
)  # INT8 weights + FP32 activations (dynamic/weight-only INT8, no calibration)
FP32_UNSUPPORTED_FORMATS = frozenset(
    {"edgetpu", "imx", "rknn", "axelera", "deepx", "qnn"}
)
...
def validate_args(format, passed_args, valid_args):
    ...
    if (
        passed_args.quantize is not None
    ):  # 32/None (FP32) is universal except FP32_UNSUPPORTED_FORMATS
        ...
        if passed_args.quantize == 16:  # FP16
            assert (
                format in FP16_FORMATS
            ), f"ERROR ❌️ quantize=16 (FP16) is not supported; {hint}"
        elif passed_args.quantize == 8:  # INT8
            assert (
                format in INT8_FORMATS
            ), f"ERROR ❌️ quantize=8 (INT8) is not supported; {hint}"
        elif (
            passed_args.quantize == "w8a16"
        ):  # INT8 weights + 16-bit activations (FP16; INT16 on LiteRT)
            assert (
                format in W8A16_FORMATS
            ), f"ERROR ❌️ quantize='w8a16' is not supported; {hint}"
        elif (
            passed_args.quantize == "w8a32"
        ):  # INT8 weights + FP32 activations (dynamic/weight-only INT8)
            assert (
                format in W8A32_FORMATS
            ), f"ERROR ❌️ quantize='w8a32' is not supported; {hint}"
        elif passed_args.quantize == 32:  # FP32
            assert (
                format not in FP32_UNSUPPORTED_FORMATS
            ), f"ERROR ❌️ quantize=32 (FP32) is not supported; {hint}"
...
def try_export(inner_func):
    ...
    def outer_func(*args, **kwargs):
        ...
        try:
            ...
            assert (
                mb > 0.1
            ), f"{mb:.3f} MB output model too small (likely corrupt or unsupported ops)"
            LOGGER.info(
...
class Exporter:
    ...
    def __call__(self, model=None) -> str:
        ...
        if fmt in {
            "tflite",
            "tfjs",
        }:  # deprecated formats, replaced by the unified Google LiteRT export
            LOGGER.warning(
        ...
        if fmt in {
            ...
            "edgetpu",
            "qnn",
        } and self.args.quantize not in {8, "w8a16"}:
        ...
        if hasattr(model, "end2end"):
            ...
            if fmt == "litert" and self.args.quantize in {8, "w8a16"}:
                ...
                model.end2end = False
                LOGGER.warning(
                    "LiteRT INT8 export does not support end2end models, disabling end2end branch."
                )
            if fmt == "engine":
                try:
        ...
        if (
            ...
            raise ValueError(
                "FP16 TorchScript export is only supported on GPU, i.e. use device=0."
            )
        ...
        if fmt == "rknn":
            ...
            if (
                self.args.name in {"rv1103", "rv1106", "rv1103b", "rv1106b"}
                ...
                LOGGER.warning(
                    f"Rockchip target '{self.args.name}' requires INT8 quantization, enabling it."
                )
                self.args.quantize = 8
        ...
        if fmt == "qnn":
            if not self.args.name:
                ...
                self.args.name = "73"
            self.args.name = (
                str(self.args.name).lower().lstrip("v")
            )  # accept '73' or 'v73'
        ...
        if self.args.nms and model.task == "semantic":
            LOGGER.warning(
                "'nms=True' is not valid for semantic segmentation models. Forcing 'nms=False'."
            )
            self.args.nms = False
        if self.args.nms:
        ...
        for m in model.modules():
            if isinstance(m, (Classify, SemanticSegment)):
                ...
                if isinstance(m, SemanticSegment) and fmt == "engine":
                    ...
                    m.bake_argmax = check_version(
                        f"tensorrt-cu{cuda_major}", ">=10.0.0"
                    ) or check_version("tensorrt", ">=10.0.0")
            if isinstance(
        ...
        if self.dla is not None:
            self.metadata["dla"] = (
                self.dla
            )  # make sure `AutoBackend` uses correct dla device if it has one
        if model.task == "pose":
        ...
        if f:
            ...
            q = (
                "quantize=16" if self.args.quantize == 16 else ""
            )  # FP16 inference flag for the val/predict hint
            LOGGER.info(
    ...
    def get_int8_calibration_dataloader(self, prefix=""):
        ...
        if self.model.task == "classify":
            ...
            dataset = ClassificationDataset(
                data[self.args.split or "val"], args=cfg, augment=False
            )
            # INT8 backends divide images by 255, so emit uint8 [0, 255] center-cropped like classify inference
            dataset.torch_transforms = T.Compose(
                [T.Resize(cfg.imgsz), T.CenterCrop(cfg.imgsz), T.PILToTensor()]
            )
        else:
        ...
        if hasattr(dataset, "transforms") and hasattr(
            dataset.transforms.transforms[0], "new_shape"
        ):
            dataset.transforms.transforms[0].new_shape = (
                self.imgsz
            )  # LetterBox with non-square imgsz
        n = len(dataset)
        if n < 1:
            raise ValueError(
        ...
        return build_dataloader(
            dataset, batch=batch, workers=0, drop_last=True
    ...
    def export_onnx(self, prefix=colorstr("ONNX:")):
        ...
        if self.args.simplify or (
            self.args.format == "onnx" and self.args.quantize == 8
        ):
        ...
        with arange_patch(
            dynamic=bool(dynamic), quantize=self.args.quantize, fmt=self.args.format
        ):
            torch2onnx(
    ...
    def export_rknn(self, prefix=colorstr("RKNN:")):
        ...
        self.args.opset = min(
            self.args.opset or 19, 19
        )  # rknn-toolkit expects opset<=19
        f_onnx = self.export_onnx()
        output_dir = Path(
            str(self.file).replace(self.file.suffix, f"_rknn_model{os.sep}")
        )
        rknn_dataset = None
        if self.args.quantize == 8:
            ...
            if not image_paths:
                raise ValueError(
                    "RKNN INT8 export requires a calibration dataset with image file paths."
                )
...
class QNNModel(ExportWrapper):
    ...
    def forward(self, x):
        ...
        return self._model(
            x.permute(0, 3, 1, 2)
        )  # the wrapped model is NCHW; the transpose folds into the NPU graph
            ...
            nms_fn = (
                partial(
                    ...
                    use_triu=not (
                        ...
                        or (
                            self.args.format == "openvino" and self.args.quantize == 8
