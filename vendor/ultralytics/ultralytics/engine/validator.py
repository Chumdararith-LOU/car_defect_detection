...
class BaseValidator:
    ...
    def __call__(self, trainer=None, model=None):
        ...
        if self.training:
            ...
            self.args.quantize = (
                16 if (self.device.type != "cpu" and trainer.amp) else None
            )
        ...
        for batch_i, batch in enumerate(bar):
            ...
            with autocast(
                self.training and self.args.quantize == 16, device=self.device.type
            ):
                # Inference
                with dt[1]:
    ...
    def match_predictions(
        ...
        for i, threshold in enumerate(self.iouv.cpu().tolist()):
            if use_scipy:
                ...
                if cost_matrix.any():
                    labels_idx, detections_idx = linear_sum_assignment(
                        -cost_matrix
                    )  # negate to maximize IoU
    ...
    def on_plot(self, name, data=None):
        ...
        if plot_type and any(
            (v.get("data") or {}).get("type") == plot_type for v in self.plots.values()
        ):
