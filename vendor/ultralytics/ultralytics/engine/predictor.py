...
class BasePredictor:
    ...
    def __init__(
        ...
        self.windows = []
        self.screen = (
            None  # cached screen resolution (width, height) for show=True scaling
        )
        self.batch = None
        self.results = None
    ...
    def setup_model(self, model, verbose: bool = True):
        ...
        self.args.quantize = (
            16 if self.model.fp16 else None
        )  # record actual inference precision
    ...
    def show(self, p: str = ""):
        ...
        im = self.plotted_img
        if (
            platform.system() in {"Linux", "Windows"} and p not in self.windows
        ):  # macOS scales natively
            self.windows.append(p)
            name = p.encode(
                "unicode_escape"
            ).decode()  # match patched cv2.imshow window name
            cv2.namedWindow(
                name, cv2.WINDOW_NORMAL | cv2.WINDOW_KEEPRATIO
            )  # allow window resize and scaling
            h, w = im.shape[:2]
            try:  # size window to fit screen once on creation if image larger than screen resolution
                ...
                cv2.resizeWindow(
                    name, max(1, int(w * r)), max(1, int(h * r))
                )  # (width, height)
            except Exception:
