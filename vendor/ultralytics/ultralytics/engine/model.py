...
class Model(torch.nn.Module):
    ...
    def __init__(
        ...
        ] = ":4096:8"  # to avoid deterministic warnings
    ...
    def benchmark(self, data=None, format="", verbose=False, **kwargs: Any):
        ...
        fmts = export_formats()
        export_args = set(
            dict(zip(fmts["Argument"], fmts["Arguments"])).get(format.lower(), [])
        ) - {
            "batch",
            "data",
            "quantize",
        ...
        export_kwargs = {
            k: v for k, v in args.items() if k in export_args
        }  # quantize is passed explicitly below
        return benchmark(
            model=self,
    ...
    def train(
        ...
        **kwargs: Any,
        ...
        }  # prioritizes rightmost args
        if isinstance(
            args.get("data"), (list, tuple)
        ):  # fine-tune a single base model across multiple datasets
            from ultralytics.engine.trainer import MultiTrainer

            use_python_trainer = (
                trainer is not None
                or self.callbacks != callbacks.get_default_callbacks()
            )
            self.trainer = MultiTrainer(
                (
                    (trainer or self._smart_load("trainer"))
                    if use_python_trainer
                    else None
                ),
                args,
                self.model,
        ...
        if args.get("resume"):
            if (
                args["resume"] is True
            ):  # resume=True (boolean) uses current model as checkpoint
                if (
                    self.ckpt
    ...
    def names(self) -> dict[int, str]:
        ...
        if (
            ...
            predictor.setup_model(
                model=self.model, verbose=False
            )  # do not mess with self.predictor.model args
