...
class Tuner:
    ...
    def _mutate(
        ...
        if x is not None:
            ...
            hyp = {
                k: float(genes[i] * factors[i]) for i, k in enumerate(self.space.keys())
            }
        else:
    ...
    def __call__(self, iterations: int = 10, cleanup: bool = True):
        ...
        for i in range(start, iterations):
            ...
            for j, (d, dataset) in enumerate(zip(data, dataset_names)):
                ...
                try:
                    train_args["data"] = d
                    train_args["save_dir"] = str(
                        save_dir[j]
                    )  # pass save_dir to subprocess to ensure same path is used
            ...
            if best_is_current:
                ...
                for s in save_dir:
                    shutil.rmtree(
                        s, ignore_errors=True
                    )  # remove iteration dirs to reduce storage space
                best_save_dirs = current_best_save_dirs
