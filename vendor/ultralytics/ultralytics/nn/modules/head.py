...
class Detect(nn.Module):
    ...
    def get_topk_index(
        ...
        idx = ori_index[
            torch.arange(batch_size)[..., None], index // nc
        ]  # original index
...
class YOLOESegment(YOLOEDetect):
    ...
    def forward_lrpc(self, x: list[torch.Tensor]) -> torch.Tensor | tuple:
        ...
        preds = dict(
            ...
            index=index,
            mask_coefficient=(
                mc * index.int() if self.export and not self.dynamic else mc[..., index]
            ),
        )
        y = self._inference(preds)
        if self.end2end:
...
class YOLOESegment26(YOLOESegment):
    ...
    def forward(
        ...
        preds = outputs[1] if isinstance(outputs, tuple) else outputs
        proto = self.proto(
            [xi.detach() for xi in x], return_semantic=False
        )  # mask protos
...
class RTDETRDecoder(nn.Module):
    ...
    def postprocess(self, boxes: torch.Tensor, scores: torch.Tensor) -> torch.Tensor:
        ...
        boxes = boxes.gather(
            dim=1, index=query_idx.unsqueeze(-1).expand(-1, -1, 4).long()
        )
        return torch.cat(
            [
                boxes,
                scores[..., None],
                (index - query_idx * self.nc)[..., None].float(),
            ],
            dim=-1,
        )
...
class SemanticSegment(nn.Module):
    ...
    def __init__(self, nc=19, ch=()):
        ...
        self.aux_head = (
            nn.Sequential(Conv(ch[1], c_mid, 3), nn.Conv2d(c_mid, nc, 1))
            if len(ch) > 1
            else None
        )
    ...
    def forward(self, x):
        ...
        if self.export:
            y = F.interpolate(
                logits, scale_factor=8, mode="bilinear", align_corners=False
            )  # [B, nc, H, W]
            ...
            if self.format in {"onnx", "mnn"} or (
                self.format == "engine" and self.bake_argmax
            ):
