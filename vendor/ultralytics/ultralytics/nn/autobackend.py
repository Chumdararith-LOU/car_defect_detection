...
def check_class_names(names: list | dict) -> dict[int, str]:
    ...
    if isinstance(names, dict):
        ...
        if isinstance(names[0], str) and names[0].startswith(
            ...
            names_map = YAML.load(ROOT / "cfg/datasets/ImageNet.yaml")[
                "map"
...
class AutoBackend(nn.Module):
    ...
    def warmup(self, imgsz: tuple[int, int, int, int] = (1, 3, 640, 640)) -> None:
        ...
        if self.format in {
            ...
            for _ in range(2 if self.format == "torchscript" else 1):
                ...
                warmup_boxes = torch.rand(
                    1, 84, 16, device=self.device
                )  # 16 boxes works best empirically
    ...
    def _model_type(p: str = "path/to/model.pt", dnn: bool = False) -> str:
        ...
        if name.endswith(
            "_qnn.onnx"
        ):  # QNN context-binary file otherwise matches the plain '.onnx' suffix
            format = "qnn"
    ...
    def _apply(self, fn) -> AutoBackend:
        ...
        if hasattr(self.backend, "model") and isinstance(self.backend.model, nn.Module):
            ...
            self.backend.device = next(
                self.backend.model.parameters()
            ).device  # update device after move
        return self
