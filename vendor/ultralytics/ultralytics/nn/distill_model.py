...
class DistillationModel(nn.Module):
    ...
    def __init__(self, teacher_model: str | Path | nn.Module, student_model: nn.Module):
        ...
        for student_out, teacher_out in zip(student_output[:-1], teacher_output[:-1]):
            ...
            projectors.append(
                nn.Sequential(
                    nn.Conv2d(
                        student_dim, teacher_dim, kernel_size=1, stride=1, padding=0
                    ),
                    nn.ReLU(inplace=True),
                    nn.Conv2d(
                        teacher_dim, teacher_dim, kernel_size=1, stride=1, padding=0
                    ),
                )
            )
    ...
    def loss(self, batch, preds=None):
        ...
        if not self.training:  # for loss calculation during validation while training
            ...
            return torch.cat([regular_loss, loss_distill]), torch.cat(
                [regular_loss_detach, loss_distill]
            )
        ...
        parts = torch.split(
            teacher_scores, [f.shape[-2] * f.shape[-1] for f in neck_feats], dim=-1
        )
        teacher_scores = tuple(
            p.sigmoid().max(dim=1, keepdim=True).values for p in parts
        )
        for i, feat_idx in enumerate(self.feats_idx[:-1]):
            ...
            student_feat = self.projector[i](
                self.decouple_outputs(self._student_feats[feat_idx])
            )
            loss_distill += (
                self.loss_sl2(
                    student_feat,
        ...
        return torch.cat([regular_loss, loss_distill]), torch.cat(
            [regular_loss_detach, distill_loss_detach]
        )
