...
class YOLOEModel(DetectionModel):
    ...
    def loss(self, batch, preds=None):
        ...
        """
        print(
            f"[DEBUG] SegmentationModel.loss called, use_objectness={getattr(self.args, 'use_objectness', False)}"
        )
        if not hasattr(self, "criterion"):
...
class _SafeLoad:
    ...
    _local = (
        threading.local()
    )  # per-thread flag set while a weights_only load is in progress
    ...
    def activation(act):
        ...
        try:
            ...
            while isinstance(
                node, ast.Attribute
            ):  # unwind e.g. torch.nn.SiLU -> ["SiLU","nn","torch"]
            ...
            assert attrs[1:] in (
                ["nn"],
                ["nn", "torch"],
            ), "activation must be a torch.nn class"
            ...
            raise TypeError(
                emojis(
                    f"ERROR ❌️ unsupported activation '{act}' blocked during restricted model load."
                )
            ) from e
    ...
    def _build(cls):
        ...
        def _getattr(
            obj, name
        ):  # ckpts pickle `Detect.forward` and `InterpolationMode.BILINEAR` via getattr
            if (
                isinstance(obj, type)
                and not name.startswith("__")
                and issubclass(obj, (nn.Module, enum.Enum))
            ):
                return getattr(obj, name)
            raise pickle.UnpicklingError(
                f"unsafe getattr({obj!r}, {name!r}) blocked during restricted model load"
            )
        ...
        allow += [
            ...
            (
                _getattr,
                "builtins.getattr",
            ),  # non-det YOLOv8, YOLO11 ckpts (restrict to nn.Module attrs)
        ]
        if WINDOWS:
...
def torch_safe_load(weight, safe_only=None):
    ...
    try:
        ...
        if (
            "PytorchStreamReader" not in str(e)
            or str(weight) != name
            or name not in GITHUB_ASSETS_NAMES
        ):
            raise
...
def load_checkpoint(weight, device=None, inplace=True, fuse=False):
    ...
    model.pt_path = str(
        weight
    )  # attach *.pt file path to model as string (avoids WindowsPath pickle issues)
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(
        ...
        if restricted and not (isinstance(m, type) and issubclass(m, torch.nn.Module)):
            ...
            raise TypeError(
                emojis(
                    f"ERROR ❌️ module '{m}' is not a permitted model layer under restricted loading."
                )
            )
