...
class LiteRTBackend(BaseBackend):
    ...
    def forward(self, im: torch.Tensor) -> list[np.ndarray]:
        ...
        kpt_start = 4 + len(
            self.names
        )  # pose keypoints follow the box (4) and class-score (nc) channels
        y = []
        for output in self.output_details:
            ...
            if output["dtype"] in {
                np.int8,
                np.int16,
            }:  # dequantize full-integer .tflite outputs (see input note)
            ...
            if x.ndim == 3 and not self.end2end:
                ...
                if (
                    self.task == "pose"
                ):  # post-NMS [B, N, box(4)+conf+cls+kpts], keypoints start at 6
                    x[:, :, 6::3] *= w
                    x[:, :, 7::3] *= h
            y.append(x)
