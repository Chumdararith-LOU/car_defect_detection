...
class OCSortTrack(STrack):
    ...
    def apply_oru(
        self, new_observation_xyxy: np.ndarray, current_frame_id: int
    ) -> None:
        ...
        for t in range(1, gap):
            ...
            self.mean, self.covariance = self.kalman_filter.update(
                self.mean, self.covariance, virtual_xyah
            )
...
class OCSORT(BYTETracker):
    ...
    def init_track(self, results, img: np.ndarray | None = None) -> list[OCSortTrack]:
        ...
        return [
            OCSortTrack(xywh, s, c, self.delta_t)
    ...
    def _ocr_associate(
        ...
        matches, u_track, u_det = matching.linear_assignment(
            ocr_dists, thresh=self.args.match_thresh
        )
    ...
    def _post_first_association(
        ...
        u_t1, u_d1 = self._ocr_associate(
            [strack_pool[i] for i in tracked], ocr_dets, activated, refind
        )
        remaining = [ocr_dets[j] for j in u_d1]
        u_t2, u_d2 = self._ocr_associate(
            [strack_pool[i] for i in other], remaining, activated, refind
        )
        ...
        if not self.use_byte:
            for i in u_track:
                ...
                if track.state == TrackState.Tracked:
                    ...
                    lost.append(track)
            return
        super()._second_association(
            strack_pool, u_track, detections_second, activated, refind, lost
        )

    def _velocity_direction_cost(
        self, tracks: list[OCSortTrack], detections: list[OCSortTrack]
    ) -> np.ndarray:
        ...
        det_centers = np.array(
            [OCSortTrack._xyxy_center(det.xyxy) for det in detections], dtype=np.float32
        )
    ...
    def _ocr_distance(
        ...
        if tracks and tracks[0].angle is not None:
            ...
            atlbrs = [
                t.last_observation if t.last_observation[0] >= 0 else t.xyxy
                for t in tracks
            ]
