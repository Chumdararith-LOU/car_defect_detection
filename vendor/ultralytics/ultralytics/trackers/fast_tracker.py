...
class FASTTracker(BYTETracker):
    ...
    def __init__(self, args):
        ...
        self.reset_velocity_offset_occ = int(
            getattr(args, "reset_velocity_offset_occ", 5)
        )
        ...
        self.active_occ_to_lost_thresh = int(
            getattr(args, "active_occ_to_lost_thresh", 10)
        )
        ...
        self._history_len = (
            max(self.reset_velocity_offset_occ, self.reset_pos_offset_occ) + 4
        )
    ...
    def _init_new_tracks(
        ...
        active_boxes.extend(
            t.xyxy for t in self.tracked_stracks if t.state == TrackState.Tracked
        )
        suppress_on = self.init_iou_suppress < 1.0
        active_stack = (
            np.asarray(active_boxes, dtype=np.float32)
            if active_boxes
        ...
        for inew in u_detection:
            ...
            if suppress_on and len(active_stack):
                if (
                    bbox_ioa(det.xyxy[None, :], active_stack, iou=True).max()
                    >= self.init_iou_suppress
                ):
                    continue
    ...
    def _remove_stale_lost(self, removed: list[STrack]) -> None:
        ...
        for track in self.lost_stracks:
            ...
            if not recently_occluded and (
                self.frame_id - track.end_frame > self.max_frames_lost
            ):
                track.mark_removed()
    ...
    def _handle_occlusions(self, r_tracked, u_track, activated_stracks, lost_stracks):
        ...
        if active_boxes.size and unmatched_boxes.size:
            cov = bbox_ioa(
                active_boxes, unmatched_boxes
            )  # (A, U) = intersection / unmatched-track area
            ...
            max_cov = cov.max(
                axis=0
            )  # per unmatched track, max fraction of its area covered by an active track
        else:
        ...
        for i, track in enumerate(unmatched):
            ...
            if (
                max_cov[i] > self.occ_cover_thresh
            ...
            if track.was_recently_occluded and (
                self.frame_id - track.last_occluded_frame > self.occ_reappear_window
            ):
            ...
            if track.state != TrackState.Lost:
                ...
                if track.not_matched > 2 and (
                    not track.is_occluded
                    or track.occluded_len > self.active_occ_to_lost_thresh
                ):
                    track.mark_lost()
