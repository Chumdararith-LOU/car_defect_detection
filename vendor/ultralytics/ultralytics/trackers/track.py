...
def on_predict_start(predictor: object, persist: bool = False) -> None:
    ...
    if hasattr(
        predictor, "_orig_postprocess"
    ):  # restore any raw-preds wrapper left by a prior TRACKTRACK run
    ...
    for _ in range(predictor.dataset.bs):
        ...
        ):  # non-stream modes reuse a single tracker
            break
    predictor.trackers = trackers
    predictor.vid_path = [
        None
    ] * predictor.dataset.bs  # used to reset the tracker when switching videos
...
def on_predict_postprocess_end(predictor: object, persist: bool = False) -> None:
    ...
    dets_del_list = (
        tracker_cls.compute_frame_extras(predictor)
        if hasattr(tracker_cls, "compute_frame_extras")
        else None
    )
