...
class ReID:
    ...
    def __init__(
        ...
        if self.is_pt:
            ...
            self.model = AutoBackend(
                str(model), device=self.device, fp16=fp16, verbose=False
            )
    ...
    def _crops_to_tensor(self, img: np.ndarray, dets: np.ndarray) -> torch.Tensor:
        ...
        for i, c in enumerate(crops):
            t = (
                torch.from_numpy(np.ascontiguousarray(c[..., ::-1]))
                .permute(2, 0, 1)
                .unsqueeze(0)
                .float()
                / 255.0
            ...
            for s in range(0, n, bs):
                ...
                if chunk.shape[0] < bs:
                    chunk = torch.cat(
                        [
                            chunk,
                            chunk[-1:].expand(bs - chunk.shape[0], *chunk.shape[1:]),
                        ],
                        0,
                    )
...
def build_encoder(
    with_reid: bool, model: str | None, device: str | torch.device | None = None
):
...
def smooth_feature(
    ...
    if (
        norm < 1e-12
    ):  # zero-norm feature has no appearance info; signal the caller to keep its current features
        return None, smooth
    feat = feat / norm
    if smooth is None:
