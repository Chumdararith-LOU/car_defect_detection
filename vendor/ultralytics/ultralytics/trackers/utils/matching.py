...
def embedding_distance(tracks: list, detections: list) -> np.ndarray:
    ...
    track_features = np.asarray(
        [f if f is not None else zeros for f in track_feats], dtype=np.float32
    )
    det_features = np.asarray(
        [f if f is not None else zeros for f in det_feats], dtype=np.float32
    )
    ...
    cost_matrix = 1 - track_features @ det_features.T / np.maximum(
        track_norm * det_norm, np.finfo(float).eps
    )
    ...
    if missing_t:
        cost_matrix[missing_t] = (
            2.0  # max cosine distance -> caller's /2 then appearance gate ignores the pair
        )
    if missing_d:
