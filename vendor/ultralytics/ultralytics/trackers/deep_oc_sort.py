...
class DeepOCSORT(OCSORT):
    ...
    def init_track(
        self, results, img: np.ndarray | None = None
    ...
    def _input_for(
        self, img: np.ndarray | None, feats: np.ndarray | None, mask: np.ndarray
    ) -> np.ndarray | None:
        ...
        """
        use_native = (
            self.encoder is not None and getattr(self.args, "model", "auto") == "auto"
        )
        if use_native:
    ...
    def _pre_first_associate(
        ...
        try:
            warp = self.gmc.apply(
                img, results_high.xyxy if len(results_high) else np.empty((0, 4))
            )
        except Exception:
            warp = np.eye(2, 3)
