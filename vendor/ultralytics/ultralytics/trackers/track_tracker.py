...
def _hmiou_distance(
    tracks_a: list[TTSTrack], tracks_b: list[TTSTrack]
...
def _angle_distance(
    tracks: list[TTSTrack], dets: list[TTSTrack], frame_id: int, delta_t: int = 3
) -> np.ndarray:
...
def _iterative_associate(
    cost: np.ndarray, match_thr: float, reduce_step: float = 0.05
) -> tuple[list]:
...
def attach_raw_preds_hook(predictor) -> None:
    ...
    def _wrapped(preds, img, orig_imgs, *args, **kwargs):
        ...
        predictor._raw_preds = (
            preds.detach().to("cpu", copy=True)
...
def _cosine_distance(tracks: list[TTSTrack], dets: list[TTSTrack]) -> np.ndarray:
    ...
    if not tracks or not dets:
        return np.ones((len(tracks), len(dets)), dtype=np.float32)
    tfeat = [
        t.smooth_feat if t.smooth_feat is not None else t.curr_feat for t in tracks
    ]
    ...
    valid = (
        np.array([f is not None for f in tfeat])[:, None]
        & np.array([f is not None for f in dfeat])[None, :]
    )
...
class TTSTrack(BOTrack):
    ...
    def __init__(
        self, xywh: np.ndarray, score: float, cls: Any, feat: np.ndarray | None = None
    ):
        ...
        super().__init__(
            xywh, score, cls
        )  # BOTrack sets smooth_feat/curr_feat and the XYWH Kalman state
    ...
    def re_activate(self, new_track, frame_id: int, new_id: bool = False) -> None:
        ...
        self.score = (
            new_track.score
        )  # set before update_features so the EMA weight uses the current confidence
...
class TRACKTRACK:
    ...
    def __init__(self, args):
        ...
        self.encoder = build_encoder(
            getattr(args, "with_reid", False),
            getattr(args, "model", "auto"),
    ...
    def _cost_matrix(self, tracks: list[TTSTrack], dets: list[TTSTrack]) -> np.ndarray:
        ...
        if self.encoder is not None:
            ...
            cost = np.where(
                np.isnan(cos),
                hmiou_dist,
                self.iou_weight * hmiou_dist + self.reid_weight * cos,
            )
        else:
            cost = hmiou_dist
    ...
    def _apply_gmc(
        self, img: np.ndarray, detections: list, pools: list[list[TTSTrack]]
    ) -> None:
    ...
    def update(
        self, results, img: np.ndarray | None = None, dets_del=None, **kwargs
    ) -> np.ndarray:
        ...
        def _new_track(box, score, cls, feat=None):
            track = (
                TTSTrack(box, score, cls, feat)
                if feat is not None
                else TTSTrack(box, score, cls)
            )
            track.min_track_len = self.min_track_len
            return track

        high_boxes, high_scores, high_cls = (
            boxes[high_mask],
            scores[high_mask],
            results.cls[high_mask],
        )
        ...
        if self.encoder is not None and len(high_boxes) > 0:
            if use_native:
                encoder_input = (
                    feats[high_mask] if (feats is not None and len(feats)) else None
                )
        ...
        if encoder_input is not None:
            ...
            dets_high = [
                _new_track(b, s, c, f)
                for b, s, c, f in zip(high_boxes, high_scores, high_cls, features)
            ]
        else:
            dets_high = [
                _new_track(b, s, c)
                for b, s, c in zip(high_boxes, high_scores, high_cls)
            ]
        dets_low = [
            _new_track(b, s, c)
            for b, s, c in zip(boxes[low_mask], scores[low_mask], results.cls[low_mask])
        ]
        ...
        if dets_del is not None:
            ...
            if mask.any():
                del_boxes = np.concatenate(
                    [del_xywh[mask], -np.ones((mask.sum(), 1))], axis=-1
                )
                dets_recovered = [
                    _new_track(b, s, c)
                    for b, s, c in zip(del_boxes, del_conf[mask], del_cls[mask])
                ]
        ...
        for det, ok in zip(
            leftover,
            _track_aware_nms(active, leftover, self.tai_thr, self.new_track_thresh),
        ):
            if ok:
