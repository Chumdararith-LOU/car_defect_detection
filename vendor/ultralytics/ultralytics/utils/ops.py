...
def process_mask(protos, masks_in, bboxes, shape, upsample: bool = False):
    ...
    if (
        masks_in.shape[0] == 0
    ):  # no detections: F.interpolate below rejects an empty (N=0) batch
        return torch.zeros(
            (0, *(shape if upsample else (mh, mw))),
            dtype=torch.uint8,
            device=masks_in.device,
        )
...
def process_mask_native(protos, masks_in, bboxes, shape):
    ...
    coeffs = masks_in @ protos.float().view(
        c, -1
    )  # (N, mh*mw) prototype-resolution mask logits
    ...
    masks = [
        scale_masks(coeffs[i : i + step].view(-1, mh, mw)[None], shape)[0]
        .gt_(0.0)
        .byte()
...
def scale_masks(
    ...
    if (
        masks.shape[1] == 0
    ):  # empty mask stack: F.interpolate rejects a 0-length channel dim
    ...
    if ratio_pad is None:  # calculate from im0_shape
        ...
        pad_w, pad_h = (im1_w - round(im0_w * gain)), (
    ...
    return F.interpolate(
        masks[..., top:bottom, left:right].float(), shape, mode=mode
    )  # NCHW masks
...
def scale_coords(
    ...
    if ratio_pad is None:  # calculate from img0_shape
        ...
        pad = (img1_w - round(img0_w * gain)) / 2, (
            img1_h - round(img0_h * gain)
        ) / 2  # wh padding
    else:
        gain = ratio_pad[0][0]
        pad = ratio_pad[1]
...
def linear_sum_assignment(cost_matrix):
    ...
    if (
        _assignment_solver is None
    ):  # resolve the backend once, then reuse it on every later call
        try:
            from scipy.optimize import (
                linear_sum_assignment as solver,
            )  # faster compiled C++ solver when installed
...
def _linear_sum_assignment_numpy(a):
    ...
    p, way = np.zeros(m + 1, np.intp), np.zeros(
        m + 1, np.intp
    )  # column->row matches and path pointers
