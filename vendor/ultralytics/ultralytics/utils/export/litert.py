...
class _NormalizeCoords(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        )  # segment returns (detections, protos)
        box_wh = torch.tensor(
            [self.w, self.h, self.w, self.h], dtype=det.dtype, device=det.device
        ).view(1, 4, 1)
        ...
        if self.task == "pose" and self.kpt_shape:
            ...
            b, _, a = det.shape
            kpts = det[:, 4 + self.nc :].view(
                b, self.kpt_shape[0], self.kpt_shape[1], a
            )
            kpt_wh = torch.tensor(
                [self.w, self.h], dtype=det.dtype, device=det.device
            ).view(1, 1, 2, 1)
            kpts = torch.cat(
                [kpts[:, :, :2] / kpt_wh, kpts[:, :, 2:]], dim=2
            )  # normalize x, y; keep conf
...
def torch2litert(
    ...
    quant_tag = (
        "_int8"
        if static_int8
        else "_w8a16" if static_int16 else "_w8a32" if dynamic_int8 else ""
    )
    ...
    if static_int8 or static_int16 or dynamic_int8:
        ...
        if (
            static_int8 or static_int16
        ):  # static schemes calibrate over representative images
            act = "int8" if static_int8 else "int16"
            LOGGER.info(
                f"{prefix} applying static quantization (int8 weights + {act} activations)..."
            )
            calib_samples = []
            for batch in calibration_dataset:
                ...
                if imgs.shape[0] < im.shape[0]:
                    imgs = imgs.repeat(-(-im.shape[0] // imgs.shape[0]), 1, 1, 1)[
                        : im.shape[0]
                    ]
                calib_samples.append({"args_0": imgs.numpy()})
            qt.load_quantization_recipe(
                recipe.static_wi8_ai8() if static_int8 else recipe.static_wi8_ai16()
            )
            ...
            qt.quantize(calibration_result=result).export_model(
                str(tflite_file), overwrite=True
            )
        else:  # dynamic / weight-only INT8: int8 weights, FP32 activations, no calibration needed
            LOGGER.info(
                f"{prefix} applying dynamic INT8 quantization (int8 weights + FP32 activations)..."
            )
