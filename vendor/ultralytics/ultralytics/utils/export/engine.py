...
def best_onnx_opset(onnx: types.ModuleType, cuda: bool = False) -> int:
    ...
    if TORCH_2_4:  # _constants.ONNX_MAX_OPSET first defined in torch 1.13
        ...
        if TORCH_2_9:
            opset = min(
                opset, 20
            )  # legacy TorchScript exporter caps at opset 20 in torch 2.9+
        if cuda:
...
def modelopt_quantize_onnx(
    ...
    if quantize == 8:
        ...
        LOGGER.info(
            f"{prefix} quantizing ONNX to INT8 with ModelOpt using {calib.shape[0]} calibration images..."
        )
        kwargs = (
    ...
    LOGGER.info(
        f"{prefix} converting ONNX to FP16 mixed precision with ModelOpt AutoCast..."
    )
    onnx.save(
...
def onnx2engine(
    ...
    flag = (
        0 if is_trt10 else (1 << int(trt.NetworkDefinitionCreationFlag.EXPLICIT_BATCH))
    )
    ...
    if dla is not None:
        ...
        if check_version(trt.__version__, ">=11.0.0,<11.1.0"):
            ...
            raise ValueError(
                "DLA is not supported in TensorRT 11.0; export with TensorRT 10.x to use DLA."
            )
    ...
    if dynamic:
        ...
        max_shape = (
            *shape[:2],
            *(int(max(2, workspace or 2) * d) for d in shape[2:]),
        )  # max input shape
        for inp in inputs:
            profile.set_shape(inp.name, min=min_shape, opt=shape, max=max_shape)
        config.add_optimization_profile(profile)
        if (
            use_int8 and not is_trt10
        ):  # deprecated in TensorRT 10, causes internal errors
