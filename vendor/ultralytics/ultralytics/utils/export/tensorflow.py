...
def onnx2saved_model(
    ...
    try:
        ...
        check_requirements(
            ...
            else "tensorflow>=2.0.0,<=2.19.0"
        )
        import tensorflow as tf
    if not cuda:
        with contextlib.suppress(
            Exception
        ):  # fails only if TF GPUs are already initialized by earlier user code
            tf.config.set_visible_devices(
                [], "GPU"
            )  # hide GPUs so non-CUDA exports never allocate GPU memory
    check_requirements(
    ...
    check_requirements(
        (
            ...
            (
                "protobuf>=6.31.1,<7.0.0" if IS_PYTHON_MINIMUM_3_13 else "protobuf>=5"
            ),  # TF>2.19 (Python 3.13) needs protobuf>=6.31.1; cap <7 to match TF gencode and avoid PaddlePaddle segfault
        ),
    ...
    if use_int8:
        tmp_file = (
            output_dir / "tmp_tflite_int8_calibration_images.npy"
    ...
    if (
        _patched != _text
    ):  # write only when unpatched; site-packages may be read-only (pre-patched containers)
        try:
            _path.write_text(_patched)
            importlib.reload(_t)
        except (
            OSError
        ) as e:  # read-only install: continue unpatched, only TopK-containing models are affected
