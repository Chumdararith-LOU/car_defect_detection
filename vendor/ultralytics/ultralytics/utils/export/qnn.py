    ...
    try:
        ...
        if "QNNExecutionProvider" in onnxruntime.get_available_providers():
            ...
            ep_lib = capi / (
                ...
                if WINDOWS
                else "libonnxruntime_providers_qnn.so"
            )
...
def onnx2qnn(
    ...
    dims = [
        d.dim_value
        for d in onnx.load(str(onnx_file)).graph.input[0].type.tensor_type.shape.dim
    ]
    if (
        len(dims) == 4 and dims[3] in {1, 3} and dims[1] not in {1, 3}
    ):  # channel-last graph (QNNModel export)
        nchw_transform = transform_fn
        transform_fn = lambda data_item: nchw_transform(data_item).transpose(
            0, 2, 3, 1
        )  # noqa: E731
    try:
        ...
        for f in (
            pre_file,
            qdq_file,
        ):  # remove quantization intermediates; the context binary is self-contained
            f.unlink(missing_ok=True)

    if not ctx_file.exists():
        raise RuntimeError(
            f"QNN context binary was not generated at {ctx_file}. See {prefix} logs for details."
        )

    if (
        metadata
    ):  # ensure Ultralytics metadata is present in the context model (usually preserved by ONNX Runtime)
        import onnx
