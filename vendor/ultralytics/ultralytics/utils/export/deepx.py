...
def onnx2deepx(
    ...
    try:
        ...
        check_requirements(
            "dx_com", cmds="-f https://sdk.deepx.ai/release/dxcom/v2.3.0/index.html"
        )
        import dx_com
    ...
    config = {
        ...
        "default_loader": {
            ...
            ),  # JSON needs str, classify returns a Path
            "file_extensions": [
                val for x in ["jpeg", "jpg", "png"] for val in (x.lower(), x.upper())
            ],
            "preprocessings": [
                {
                    "resize": {
