...
def normalize_platform_uri(uri):
    ...
    s = str(uri)
    return (
        f"ul://{s[len(PLATFORM_URL) + 1 :].strip('/')}"
        if s.startswith(f"{PLATFORM_URL}/")
        else uri
    )
...
def parse_version(version="0.0.0") -> tuple:
    ...
    try:
        ...
        return tuple(
            nums + [0] * (3 - len(nums))
        )  # pad to 3, i.e. '2.0.1+cpu' -> (2, 0, 1), '2' -> (2, 0, 0)
...
def check_requirements(
    ...
    for r in requirements:
        ...
        for candidate in candidates:
            r_stripped = candidate.rpartition("/")[-1].replace(
                ".git", ""
...
def check_file(file, suffix="", download=True, download_dir=".", hard=True):
    ...
    if (
        ...
        if file.startswith("gs://"):
            file = (
                "https://storage.googleapis.com/" + file[5:]
...
def check_multiple_install():
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            ):  # Package not pip-installed but locally imported
                LOGGER.warning(
                    f"Ultralytics not found via pip but importing from: {ROOT}. {install_msg}"
                )
            return
        yolo_path = (
            Path(re.findall(r"location:\s+(.+)", result.stdout, flags=re.I)[-1])
            / "ultralytics"
        ).resolve()
...
def print_args(args: dict | None = None, show_file=True, show_func=False):
    ...
    LOGGER.info(
        colorstr(s) + ", ".join(f"{k}={strip_auth(v)}" for k, v in sorted(args.items()))
    )
