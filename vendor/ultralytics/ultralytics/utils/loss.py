...
class SeesawBCE(nn.Module):
    ...
    def forward(self, pred: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        if self.cum_samples is None:
            self.cum_samples = torch.zeros(
                pred.shape[-1], dtype=torch.float32, device=pred.device
            )
        if self.training:
        ...
        mitigation = (
            ((n / n.max()) ** self.p).to(pred.dtype).view([1] * (pred.dim() - 1) + [-1])
        )
        ...
        weights = torch.where(
            target == 0, mitigation * compensation, torch.ones_like(pred)
        )
...
class v8DetectionLoss:
    ...
    def __init__(
        ...
        _get = (
            ...
            else (lambda k: getattr(h, k, None))
        )
...
class v8SegmentationLoss(v8DetectionLoss):
    ...
    def __init__(
        ...
        print(
            f"[DEBUG] v8SegmentationLoss.__init__ called, use_objectness={getattr(model.args, 'use_objectness', False)}"
        )
        ...
        if loss_type == "focal":
            try:
                ...
                print(
                    "[⚠️] Could not import ScaledFocalBCEWithLogitsLoss from src.models.losses. Falling back to standard BCE."
                )
    ...
    def __call__(
        ...
        """Calculate the sum of the loss for box, cls and dfl multiplied by batch size."""
        print(
            f"[DEBUG] v8SegmentationLoss.__call__ called, use_objectness={getattr(self.args, 'use_objectness', False)}"
        )
        ...
        if getattr(self.args, "use_objectness", False):
            print(
                f"[DEBUG] use_objectness=True, preds_top keys: {preds_top.keys() if isinstance(preds_top, dict) else type(preds_top)}"
            )
...
class v8PoseLoss(v8DetectionLoss):
    ...
    def __init__(
        self, model: torch.nn.Module, tal_topk: int = 10, tal_topk2: int = 10
    ):  # model must be de-paralleled
    ...
    def loss(
        ...
        )  # box, kpt_location, kpt_visibility, cls, dfl
        (
            (fg_mask, target_gt_idx, target_bboxes, anchor_points, stride_tensor),
            det_loss,
            _,
...
class PoseLoss26(v8PoseLoss):
    ...
    def loss(
        ...
        if self.rle_loss and preds.get("kpts_sigma", None) is not None:
            ...
            pred_sigma = pred_sigma.view(
                batch_size, -1, self.kpt_shape[0], 2
            )  # (b, h*w, 17, 2)
...
class v8OBBLoss(v8DetectionLoss):
    ...
    def __init__(
        self, model: torch.nn.Module, tal_topk=10, tal_topk2: int | None = None
    ):
...
class E2ELoss:
    ...
    def __call__(
        ...
        if isinstance(preds_top, dict) and "obj" in preds_top:
            ...
            loss = (
                loss[0]
                + (obj_loss * getattr(self.one2many.args, "obj_loss_weight", 1.0)),
                loss[1],
            )
            loss_items = torch.cat(
                [
...
class SemanticSegmentationLoss(nn.Module):
    ...
    def __init__(self, model: torch.nn.Module):
        ...
        self.use_cityscapes_weight = data_name in {
            "cityscapes",
            "cityscapes8",
        } and self.nc == len(CITYSCAPES_WEIGHT)
        weight = getattr(
            model, "class_weights", None
        )  # cls_pw frequency weights, else hardcoded Cityscapes
        if weight is None and self.use_cityscapes_weight:
            weight = torch.from_numpy(CITYSCAPES_WEIGHT)
        weight = (
            None if weight is None else weight.to(device=self.device, dtype=self.dtype)
        )
        if self.nc == 1:
            self.ce = (
                nn.BCEWithLogitsLoss()
            )  # binary: class weighting intentionally unsupported
        else:
            self.ce = nn.CrossEntropyLoss(ignore_index=255).to(
    ...
    def _resize_masks(self, masks, target_shape):
        ...
        if masks.shape[1:] != target_shape:
            return (
                F.interpolate(
                    masks.float().unsqueeze(1), size=target_shape, mode="nearest"
                )
                .squeeze(1)
                .to(torch.int32)
            )
    ...
    def _dice_loss(self, preds, masks):
        ...
        intersection.scatter_add_(
            0, target, flat_pred.gather(1, target[:, None]).squeeze(1)
        )
        pred_sum = flat_pred.sum(dim=0)
        target_sum = torch.bincount(target, minlength=self.nc).to(
    ...
    def forward(self, preds, batch):
        ...
        if preds.shape[2:] != masks.shape[1:]:
            preds = F.interpolate(
                preds, size=masks.shape[1:], mode="bilinear", align_corners=False
            )
