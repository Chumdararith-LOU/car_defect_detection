...
class TQDM:
    ...
    NONINTERACTIVE_MIN_INTERVAL = (
        60.0  # Minimum interval for non-interactive environments
    )
    ...
    def __init__(
        ...
        bar_format: (
            str | None
        ) = None,  # kept for API compatibility; not used for formatting
        initial: int = 0,
        **kwargs,
    ) -> None:
    ...
    def _should_update(self, dt: float, dn: int) -> bool:
        ...
        if self.noninteractive:
            return False
        return (self.total is not None and self.n >= self.total) or (
            dt >= self.mininterval
        )
    ...
    def _display(self, final: bool = False) -> None:
        ...
        if self.total:
            ...
            if (
                self.is_bytes and n_str[-2] == t_str[-2]
