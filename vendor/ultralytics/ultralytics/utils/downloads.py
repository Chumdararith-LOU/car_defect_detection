...
def safe_download(
    ...
    url = str(url)
    if (
        "://" not in url and Path(url).is_file()
    ):  # local file path ('://' check required in Windows Python<3.10)
        f = Path(url)
    else:
...
def get_github_assets(
    ...
    attempts = (
        2 if retry else 1
    )  # retry once on transient network errors or non-200 responses
    for attempt in range(attempts):
        try:
            ...
            return "", []
        if (
            r.status_code == 200 or r.reason == "rate limit exceeded"
        ):  # do not retry 403 rate limits
            break
    ...
    ]  # tag, assets i.e. ['yolo26n.pt', 'yolo11s.pt', ...]
