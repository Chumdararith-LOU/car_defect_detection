...
class Annotator:
    ...
    def __init__(
        ...
        if self.pil:  # use PIL
            self.im = (
                im if input_is_pil else Image.fromarray(im)
            )  # stay in BGR since color palette is in BGR
    ...
    def show(self, title: str | None = None):
        ...
        ):  # cannot use IS_JUPYTER as it runs for all IPython environments
            try:
                display(
                    im
...
def plot_labels(boxes, cls, names=(), save_dir=Path(""), on_plot=None):
    ...
    for class_id, box in zip(cls[:500], boxes[:500]):
        ImageDraw.Draw(img).rectangle(
            box.tolist(), width=1, outline=colors(class_id)
        )  # plot
    ax[1].imshow(img)
    ax[1].axis("off")
...
def plot_images(
    ...
    for i in range(bs):
        ...
        if len(cls) > 0:
            ...
            if len(masks):
                if (
                    idx.shape[0] == masks.shape[0] and masks.max() <= 1
                ):  # overlap_mask=False
        ...
        if len(semantic_masks) and i < len(semantic_masks):
            ...
            im = np.asarray(annotator.im).copy()
            sub_annotator = Annotator(
                np.ascontiguousarray(im[y : y + h, x : x + w]), line_width=1, pil=False
            )
    ...
    for i, f in enumerate(files):
        try:
            ...
            for i, j in enumerate(columns):
                ...
                ax[i].plot(
                    x, _gaussian_filter1d(y, sigma=3), ":", label="smooth", linewidth=2
                )  # smoothing line
...
def plot_tune_results(
    ...
    plt.plot(
        x,
        _gaussian_filter1d(all_fitness, sigma=3),
        ":",
        color="0.35",
        label="smoothed mean",
        linewidth=2,
