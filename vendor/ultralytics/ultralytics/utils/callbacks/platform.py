...
def slugify(text):
    ...
    if not text:
        return text
    return re.sub(
        r"-+", "-", re.sub(r"[^a-z0-9\s-]", "", str(text).lower()).replace(" ", "-")
    ).strip("-")[:128]
...
def _sanitize_json_value(value):
    ...
    if isinstance(value, float):
        return (
            value if isfinite(value) else None
        )  # avoid "Out of range float values are not JSON compliant" warnings
    return value
...
def _send(event, data, project, name, model_id=None, retry=2):
    ...
    def post():
        ...
        if 400 <= r.status_code < 500 and r.status_code not in {408, 429}:
            ...
            return (
                None  # Don't retry client errors (except 408 timeout, 429 rate limit)
            )
        r.raise_for_status()
        return r.json()
...
def _upload_model(model_path, project, name, progress=False, retry=1, model_id=None):
    ...
    def get_signed_url():
        ...
        if model_id:
            payload["modelId"] = (
                model_id  # Direct lookup avoids slug mismatch from auto-increment
            )
        r = requests.post(
...
def on_train_end(trainer):
    ...
    for info in getattr(getattr(trainer, "validator", None), "plots", {}).values():
        if info.get("data") and info["data"].get("type"):
            plots_by_type.setdefault(
                info["data"]["type"], info["data"]
            )  # Don't overwrite trainer plots
    plots = [
        _interp_plot(p) for p in plots_by_type.values()
    ]  # Interpolate curves to reduce size
