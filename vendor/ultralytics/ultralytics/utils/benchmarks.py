...
def benchmark(
    ...
    **kwargs,
    ...
    if format_arg:
        ...
        assert (
            format_arg in formats
        ), f"Expected format to be one of {formats}, but got '{format_arg}'."
    for name, format, suffix, cpu, gpu, valid_args, _ in zip(
        *export_formats().values()
        ...
        try:
            ...
            if (
                IS_PYTHON_MINIMUM_3_13
                and not format_arg
                and format in {"saved_model", "pb", "edgetpu"}
            ):
                continue
            ...
            if format == "pb":
                ...
                assert not (
                    MACOS and IS_PYTHON_MINIMUM_3_13
            ...
            if format == "paddle":
                ...
                assert (
                    LINUX and not IS_JETSON
                ) or MACOS, "Windows and Jetson Paddle exports not supported yet"
            ...
            if format == "litert":
                assert MACOS or (
                    LINUX and not ARM64
                ), "LiteRT benchmark only supported on Linux x86 and macOS"
                ...
                assert not (
                    MACOS and is_github_action_running()
            ...
            if format == "-":
                ...
                exported_model = (
                    ...
                    else YOLO(filename, task=model.task)
                )
