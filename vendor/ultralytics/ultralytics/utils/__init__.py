...
SAFE_LOAD = (
    str(os.getenv("ULTRALYTICS_SAFE_LOAD", False)).lower() == "true"
)  # opt-in weights_only model loading
...
class YAML:
    ...
    def load(cls, file="data.yaml", append_filename=False):
        ...
        if data is None:  # empty file, comments only, or explicit 'null'
            ...
            data, dict
        ):  # reject non-mapping YAML (scalar/list) with a clear error, not a cryptic one
            raise ValueError(
