...
def _get_covariance_matrix(
    boxes: torch.Tensor, floor: float = 0.0
...
class ConfusionMatrix(DataExportMixin):
    ...
    def process_batch(
        ...
        conf = (
            0.25 if conf in {None, 0.01 if is_obb else 0.001} else conf
        ...
        for i, gc in enumerate(gt_classes):
            ...
            if n and sum(j) == 1:
                ...
                self.matrix[
                    dc, gc
                ] += 1  # TP if class is correct else both an FP and an FN
                if dc == gc:
...
class SegmentMetrics(DetMetrics):
    ...
    def process(
        ...
        Returns:
            (dict[str, np.ndarray]): Dictionary containing concatenated statistics arrays.
        """
        stats = DetMetrics.process(
            self, save_dir, plot, on_plot=on_plot
        )  # process box stats
...
class SemanticMetrics(SimpleClass, DataExportMixin):
    ...
    def update_stats(self, preds: torch.Tensor, targets: torch.Tensor) -> None:
        ...
        if self.matrix is None:
            self.matrix = torch.zeros(
                (self.cm_nc, self.cm_nc), device=preds.device, dtype=torch.float32
            )
        ...
        present = torch.zeros(
            (targets.shape[0], self.cm_nc), dtype=torch.bool, device=targets.device
        )
        batch_idx = (
    ...
    def process(
        ...
        self._pixel_accuracy = float(
            (intersection.sum() / (self.matrix.sum() + 1e-10)).item()
        )
    ...
    def _plot_iou_bars(self, save_dir, on_plot):
        ...
        names = (
            list(self.names.values())
            if self.names
            else [str(i) for i in range(self.nc)]
        )
        x = np.arange(self.nc)
        bars = ax.bar(
            x,
            self._per_class_iou,
            color=[list(c / 255.0 for c in colors(i, False)) for i in range(self.nc)],
        )
        ax.set_xlabel("Class")
        ax.set_ylabel("IoU")
