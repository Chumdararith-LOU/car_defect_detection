...
def parse_device(device: str | int | list | tuple | torch.device = "") -> str:
    ...
    if device == "cuda":
        device = "0"
    device = ",".join(
        str(int(x)) if x.isdigit() else x for x in device.split(",") if x
    )  # "0,,01" -> "0,1"
    ...
    visible = [str(int(x)) if x.isdigit() else x for x in cvd.split(",") if x][
        : torch.cuda.device_count()
    ]
    indices = [
        x for x in device.split(",") if x.isdigit()
    ]  # requested ids, excluding '-1' and non-numeric tokens
    if (
        indices
        and all(x in visible for x in indices)
        and any(int(x) >= torch.cuda.device_count() for x in indices)
    ):
        ...
        device = ",".join(
            str(visible.index(x)) if x.isdigit() else x for x in device.split(",")
        )
    if "-1" in device:
        ...
        selected = GPUInfo().select_idle_gpu(
            count=parts.count("-1"), min_memory_fraction=0.2, indices=candidates
        )
...
def select_device(device="", newline=False, verbose=True):
    ...
    if not cpu and not mps and device:  # non-cpu device requested
        valid = all(
            x.isdigit() and int(x) < torch.cuda.device_count()
            for x in device.split(",")
        )
    ...
    if not cpu and not mps and torch.cuda.is_available():  # prefer GPU if available
        devices = (
            device.split(",") if device else [str(torch.cuda.current_device())]
        ...
        if (
            device and len(devices) == 1
        ):  # explicit single-GPU request only: '' never moves the current device, and
            torch.cuda.set_device(
                int(devices[0])
            )  # multi-GPU DDP ranks each pin their own device in _setup_ddp()
