    ...
    def __call__(self, *args: Any, **kwargs: Any):
        ...
        if self.CFG["verbose"]:
            ...
            if (
                ...
                and hasattr(self.model.predictor, "imgsz")
            ):
...
class SolutionAnnotator(Annotator):
    ...
    def adaptive_label(
        ...
        x_center, y_center = int((box[0] + box[2]) / 2), int(
            (box[1] + box[3]) / 2
        )  # Bounding-box center
        ...
        if shape == "circle":
            cv2.circle(
                self.im,
                (x_center, y_center),
                int(((text_size[0] ** 2 + text_size[1] ** 2) ** 0.5) / 2)
                + margin,  # Calculate the radius
                color,
                -1,
            )
        else:
