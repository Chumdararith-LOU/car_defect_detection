...
class VisualAISearch:
    ...
    def load_or_build_index(self) -> None:
        ...
        if Path(self.index_path).exists() and Path(self.data_path_npy).exists():
            ...
            self.index = np.load(
                self.index_path
            )  # Load the L2-normalized embeddings from disk
    ...
    def search(
        ...
        scores = (
            self.index @ text_feat[0]
        )  # cosine similarity (embeddings are L2-normalized)
        top_k = np.argsort(scores)[::-1][: max(k, 0)]
        results = [
            (self.image_paths[i], float(scores[i]))
            for i in top_k
            if scores[i] >= similarity_thresh
        ]
