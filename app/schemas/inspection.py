"""Pydantic schemas for the inspection API request/response models.

Field naming: the pipeline internally uses long keys (``defect_id``,
``defect_class``, ``global_bbox_xyxy``, ``assigned_panel``,
``containment_ratio_iod``, ``damage_severity_index_dsi``); the API serializes
them to the shorter frontend-facing keys (``id``, ``class``, ``bbox``,
``panel``, ``iod``, ``dsi``) via ``serialization_alias``.
"""
...
class Stage1Blob(BaseModel):
    """Raw Stage 1 saliency blob (kept in the payload for the S1 view)."""
...
class Defect(BaseModel):
    ...
    crop_url: Optional[str] = Field(
        None, description="URL to the cropped close-up image of this defect"
    )
...
class InspectionPayload(BaseModel):
    inspection_id: str
    timestamp: str
    vehicle_color_detected: Optional[str] = "Unknown"
    total_defects_found: int
    inspection_status: str = Field(
        ..., description="Expected values: 'PASS' or 'FAIL'"
    )
